             ...
             const int options_bonflev_size[2], bool options_msg, struct_T *out)
{
  ...
  if ((options_bonflev_size[0] != 0) && (options_bonflev_size[1] != 0)) {
    ...
    for (i = 0; i <= end; i++) {
      ...
      for (i1 = 0; i1 < loop_ub; i1++) {
        b_gmin->data[i1 + b_gmin->size[0] * i] =
    ...
    if ((options_bonflev_size[0] == 0) || (options_bonflev_size[1] == 0)) {
      ...
      if (out->mdr->data[(int)ii - 1] < ndecl - 1.0) {
        ...
        if (end > 0) {
          ...
          for (i1 = 0; i1 < loop_ub; i1++) {
            gval->data[i1] = gmin1->data[i + i1];
          }
          loop_ub = vlen - i2;
          for (i = 0; i < loop_ub; i++) {
            gval->data[i + gval->size[0]] =
  ...
  if (ndecl > 0.0) {
    ...
    if (INP_bb->size[1] < d + 1.0) {
      ...
      e_eml_find(b_beta, (int *)&end, tmp_size);
               ...
               struct_T *out)
{
  ...
  if ((options_bonflev_size[0] != 0) && (options_bonflev_size[1] != 0)) {
    ...
    nmdr = 0;
    for (b_i = 0; b_i <= end; b_i++) {
      if (mdr->data[b_i + mdr->size[0]] > gmin->data[b_i + gmin->size[0]]) {
        nmdr++;
      }
    }
    emxInit_int32_T(&r3, 1);
    ...
    nmdr = 0;
    for (b_i = 0; b_i <= end; b_i++) {
      if (mdr->data[b_i + mdr->size[0]] > gmin->data[b_i + gmin->size[0] * 2]) {
        nmdr++;
      }
    }
    emxInit_int32_T(&r4, 1);
    ...
    nmdr = 0;
    for (b_i = 0; b_i <= end; b_i++) {
      if (mdr->data[b_i + mdr->size[0]] > gmin->data[b_i + gmin->size[0] * 3]) {
        nmdr++;
      }
    }
    emxInit_int32_T(&r6, 1);
    ...
    nmdr = 0;
    for (b_i = 0; b_i <= end; b_i++) {
      if (mdr->data[b_i + mdr->size[0]] > gmin->data[b_i + gmin->size[0] * 4]) {
        nmdr++;
      }
    }
    emxInit_int32_T(&r8, 1);
    ...
    nmdr = 0;
    for (b_i = 0; b_i <= end; b_i++) {
      if (mdr->data[b_i + mdr->size[0]] < gmin->data[b_i + gmin->size[0] * 5]) {
        nmdr++;
      }
    }
    emxInit_int32_T(&r9, 1);
  ...
  if (empty_non_axis_sizes || condition2) {
    ...
    while ((!exitg2) && (c_i - 2 <= nmdr - 2)) {
      ...
      if ((double)(c_i - 2) + 3.0 < (ndecl - INP_init) + 1.0) {
        ...
        /*  Three consecutive values of d_min above the 99.99% threshold or 1 */
        /*  above 99.999% envelope */
        if (((mdr->data[c_i + mdr->size[0]] >
              gmin->data[c_i + gmin->size[0] * c9999]) &&
             (mdr->data[(c_i + mdr->size[0]) + 1] >
              gmin->data[(c_i + gmin->size[0] * c9999) + 1]) &&
             (mdr->data[(c_i + mdr->size[0]) - 1] >
              gmin->data[(c_i + gmin->size[0] * c9999) - 1])) ||
            (mdr->data[c_i + mdr->size[0]] >
             gmin->data[(gmin->size[0] + gmin->size[0] * c99) - 1]) ||
            (mdr->data[c_i + mdr->size[0]] >
             gmin->data[c_i + gmin->size[0] * c99999])) {
          if (options_msg) {
            int2str(mdr->data[c_i], b_mdr.data, b_mdr.size);
          }
          if ((mdr->data[c_i + mdr->size[0]] >
               gmin->data[c_i + gmin->size[0] * c9999]) &&
              (mdr->data[(c_i + mdr->size[0]) + 1] >
            ...
            if (options_msg) {
              ...
              int2str(mdr->data[c_i - 1], b_mdr.data, b_mdr.size);
              int2str(INP_n, b_mdr.data, b_mdr.size);
              int2str(mdr->data[c_i + 1], b_mdr.data, b_mdr.size);
              int2str(INP_n, b_mdr.data, b_mdr.size);
            }
        ...
        condition3 = (mdr->data[c_i + mdr->size[0]] >
                      gmin->data[(gmin->size[0] + gmin->size[0] * c99) - 1]);
        condition4 = (mdr->data[c_i + mdr->size[0]] >
                      gmin->data[c_i + gmin->size[0] * c99999]);
        if (empty_non_axis_sizes || condition2 || condition3 || condition4) {
          if (options_msg) {
            /*  disp(['Signal in final part of the search: step '
          ...
          if (empty_non_axis_sizes) {
            if (options_msg) {
               ...
               * rmin('  int2str(mdr(i+1,1)) ',' int2str(n) ')>99.9% and rmin('
               * int2str(mdr(i-1,1)) ',' int2str(n) ')>99%']); */
              print_processing(mdr->data[c_i], INP_n, mdr->data[c_i + 1], INP_n,
                               mdr->data[c_i - 1], INP_n, validatedHoleFilling);
              Rprintf("\nrmin(%.0f,%.0f)>99.9%% and rmin(%.0f,%.0f)>99.9%% and "
                     "rmin(%.0f,%.0f)>99%%",
                     validatedHoleFilling[0], validatedHoleFilling[1],
                     validatedHoleFilling[2], validatedHoleFilling[3],
          ...
          if (condition2) {
            if (options_msg) {
               ...
               * and rmin('  int2str(mdr(i,1)) ',' int2str(n) ')>99.9% and
               * rmin('  int2str(mdr(i+1,1)) ',' int2str(n) ')>99%']); */
              print_processing(mdr->data[c_i - 1], INP_n, mdr->data[c_i], INP_n,
                               mdr->data[c_i + 1], INP_n, validatedHoleFilling);
              Rprintf("\nrmin(%.0f,%.0f)>99.9%% and rmin(%.0f,%.0f)>99.9%% and "
                     "rmin(%.0f,%.0f)>99%%",
                     validatedHoleFilling[0], validatedHoleFilling[1],
          ...
          if (condition4) {
            if (options_msg) {
              /*  disp(['rmin('  int2str(mdr(i,1)) ',' int2str(n) ')>99.999%']);
               */
              Rprintf("\nrmin(%.0f,%.0f)>99.999%%", mdr->data[c_i], INP_n);
              //fflush(stdout);
            }
            int2str(mdr->data[c_i], b_mdr.data, b_mdr.size);
            int2str(INP_n, b_mdr.data, b_mdr.size);
      ...
      } else if (((mdr->data[c_i + mdr->size[0]] >
                   gmin->data[c_i + gmin->size[0] * c999]) ||
                  (mdr->data[c_i + mdr->size[0]] >
                   gmin->data[(gmin->size[0] + gmin->size[0] * c99) - 1])) &&
                 (c_i + 1 == mdr->size[0] - 1)) {
      ...
      if (b_signal == 1) {
        ...
        /*  if the following statement is true, observed curve of r_min is */
        /*  above 99.99% and later is below 1%: peak followed by dip */
        if (mdr->size[0] > (mdag - mdr->data[0]) + 31.0) {
          for (i = 0; i < 31; i++) {
            ij = ((unsigned int)i + c_i) + 2U;
            x[i] = (mdr->data[((int)ij + mdr->size[0]) - 1] <
                    gmin->data[((int)ij + gmin->size[0] * c001) - 1]);
          }
          end = x[0];
          for (loop_ub = 0; loop_ub < 30; loop_ub++) {
        ...
        if (!NoFalseSig) {
          ...
          /* [gval]=FSRenvmdr(mdag+1,p,'prob',0.01,'m0',mdag); */
          b_FSRenvmdr(mdag + 1.0, INP_p, mdag, gval);
          if (mdr->data[c_i + mdr->size[0]] < gval->data[gval->size[0]]) {
            if (options_msg) {
    ...
    if ((options_bonflev_size[0] == 0) || (options_bonflev_size[1] == 0)) {
      ...
      while ((!exitg1) && (vlen <= i - 1)) {
        ...
        while ((!exitg2) && (end <= i1)) {
           ...
           */
          /*  of the resuperimposed envelope based on tr observations */
          d = mdr->data[((int)ii + mdr->size[0]) - 1];
          if ((d > gmin1->data[((int)ii + gmin1->size[0] * c99) - 1]) &&
              ((int)ii >= gmin1->size[0] - 2)) {
            /*  Condition S1 */
            int2str(mdr->data[(int)ii - 1], b_mdr.data, b_mdr.size);
      ...
      /*  Condition H2 */
      /*  Check if stopping rule takes place at m* <m^\dagger+k */
      if (mdr->data[(int)ii - 1] < ndecl - 1.0) {
        /*  Condition H2b and H2a */
        if ((unsigned int)ii + 1U > (unsigned int)gmin1->size[0]) {
          i = 0;
          i1 = 0;
        ...
        if (end > 0) {
          ...
          if (r10->size[0] == 1) {
            ...
            /*  Make sure that gfind(1,1)-mdr(1,1) is not smaller */
            /*  than i otherwise tr  becomes empty */
            d = gval->data[r10->data[0] - 1] - mdr->data[0];
            if (d >= b_i + 1) {
              if (b_i + 1 > d) {
                b_i = 0;
                i = 0;
  ...
  if (ndecl > 0.0) {
    ...
    if (INP_bb->size[1] < d + 1.0) {
      ...
      e_eml_find(b_beta, (int *)&vlen, tmp_size);
      emxFree_boolean_T(&b_beta);
      loop_ub = INP_bb->size[0];
      i = r2->size[0] * r2->size[1];
      r2->size[0] = INP_bb->size[0];
      ...
      if (c_ifWhileCond(&b_tmp_data)) {
        ...
        r11->size[0] = r2->size[0];
        r11->size[1] = r2->size[1];
        emxEnsureCapacity_boolean_T(r11, i);
        loop_ub = r2->size[0] * r2->size[1];
        for (i = 0; i < loop_ub; i++) {
          r11->data[i] = !r2->data[i];
        }
        end = r11->size[0] * r11->size[1] - 1;
        nmdr = 0;
        for (b_i = 0; b_i <= end; b_i++) {
          if (r11->data[b_i]) {
            nmdr++;
  ...
  if (INP_weak) {
    if (mdag > -1.0) {
      ...
      i = good->size[0] * good->size[1];
      good->size[0] = 1;
      good->size[1] = r12->size[0];
      emxEnsureCapacity_real_T(good, i);
      loop_ub = r12->size[0];
      for (i = 0; i < loop_ub; i++) {
        good->data[i] = out->ListCl->data[r12->data[i] - 1];
      }
      emxFree_int32_T(&r12);
      i = out->ListCl->size[0] * out->ListCl->size[1];
      out->ListCl->size[0] = 1;
