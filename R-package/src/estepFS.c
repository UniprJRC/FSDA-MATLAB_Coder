               ...
               emxArray_real_T *varargout1)
{
  ...
  k = varargout1->size[0];
  varargout1->size[0] = log_lh->size[0];
  emxEnsureCapacity_real_T(varargout1, k);
  nx = log_lh->size[0];
  for (k = 0; k < nx; k++) {
    varargout1->data[k] = log_lh->data[k] - maxll->data[k];
  }
  nx = varargout1->size[0];
  for (k = 0; k < nx; k++) {
    varargout1->data[k] = exp(varargout1->data[k]);
  }
  ...
  if (varargout1->size[0] == 0) {
    density->size[0] = 0;
  } else {
    nx = varargout1->size[0];
    k = density->size[0];
    density->size[0] = varargout1->size[0];
    emxEnsureCapacity_real_T(density, k);
    for (k = 0; k < nx; k++) {
      density->data[k] = varargout1->data[k];
    }
  }
  ...
  nx = varargout1->size[0];
  for (k = 0; k < nx; k++) {
    varargout1->data[k] /= density->data[k];
  }
...
double c_estepFS(const emxArray_real_T *log_lh)
{
  ...
  /*  minus maxll to avoid underflow */
  k = post->size[0];
  post->size[0] = log_lh->size[0];
  emxEnsureCapacity_real_T(post, k);
  nx = log_lh->size[0];
  for (k = 0; k < nx; k++) {
    post->data[k] = log_lh->data[k] - maxll->data[k];
  }
  nx = post->size[0];
  for (k = 0; k < nx; k++) {
  ...
  if (post->size[0] == 0) {
    ...
    nx = post->size[0];
    k = density->size[0];
    density->size[0] = post->size[0];
    emxEnsureCapacity_real_T(density, k);
    for (k = 0; k < nx; k++) {
      density->data[k] = post->data[k];
    }
  }
  emxFree_real_T(&post);
  /*  logpdf is a size(log_lh,1)-by-1 vector which contains the contributions */
             ...
             emxArray_real_T *varargout1, emxArray_real_T *varargout2)
{
  ...
  if (varargout1->size[0] == 0) {
    ...
    nx = varargout1->size[0];
    k = varargout2->size[0];
    varargout2->size[0] = varargout1->size[0];
    emxEnsureCapacity_real_T(varargout2, k);
    for (k = 0; k < nx; k++) {
      varargout2->data[k] = varargout1->data[k];
    }
  }
  /* normalize posteriors */
  /*  varargout1 is a size(log_lh,1)-by-p matrix which contains posterior */
  ...
  nx = varargout1->size[0];
  for (k = 0; k < nx; k++) {
    varargout1->data[k] /= varargout2->data[k];
  }
  /*  logpdf is a size(log_lh,1)-by-1 vector which contains the contributions */
  /*  to the loglikelihood of the mixture model of each observation */
  /*  Note that give that maxll was substracted now it must be added */
