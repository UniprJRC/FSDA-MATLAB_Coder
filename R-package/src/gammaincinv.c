...
double eml_gammaincinv(double p, double a, double la, double lga, double lgap1)
{
  ...
  if (a == rtInf) {
    ...
    if (nu < -1.24 * log(pLower)) {
      ...
      if (fabs(pLower - 0.5) <= 0.425) {
        ...
        if (r <= 5.0) {
          r -= 1.6;
          rval =
              (((((((0.00077454501427834139 * r + 0.022723844989269184) * r +
                    0.24178072517745061) *
                       r +
                   1.2704582524523684) *
                      r +
          ...
          r -= 5.0;
          rval =
              (((((((2.0103343992922881E-7 * r + 2.7115555687434876E-5) * r +
    ...
    exitg1 = false;
    while ((!exitg1) && (i < 1000)) {
      log1mpLower = (double)sgn * (eml_gammainc(rval, a, la, lgap1, upper) - p);
      if ((log1mpLower * r < 0.0) && (fabs(r) <= fabs(log1mpLower))) {
        rval = 0.5 * rval + 0.5 * oldz;
        log1mpLower =
            (double)sgn * (eml_gammainc(rval, a, la, lgap1, upper) - p);
      }
      if (log1mpLower > 0.0) {
        p1 = rval;
      } else {
      ...
      if ((fabs(log1mpLower) < pLower) ||
        ...
        guard1 = false;
        if (i < 500) {
          rval *= 1.0 - log1mpLower /
                            (rval * exp(((a - 1.0) * log(rval) - rval) - lga) +
                             log1mpLower * ((rval + 1.0) - a) / 2.0);
          if (rval <= nu) {
            if (nu == 0.0) {
