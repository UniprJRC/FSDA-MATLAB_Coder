...
static const double dv1[3601] = {0.0,
  ...
  for (il = 0; il < i; il++) {
    ...
    for (jl = 0; jl < i1; jl++) {
      ...
      if ((unsigned int)(il + 1) != b_jl) {
        ...
        /*   Em  is the median  of vector E */
        if ((n - 2 > 0) && (varargin_2 >= 0.0)) {
          brm = ((double)n - 2.0) / 2.0;
          if (varargin_2 > (betainc(0.5, 1.0, brm)).re) {
            b_a = betaincinv(varargin_2, brm, 1.0);
            mady = (1.0 - b_a.re) / b_a.re;
          } else {
            b_a = b_betaincinv(varargin_2, 1.0, brm);
            mady = b_a.re / (1.0 - b_a.re);
          }
          mady = mady * ((double)n - 2.0) / 2.0;
        } else {
