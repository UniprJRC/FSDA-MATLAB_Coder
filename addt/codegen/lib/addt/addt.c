...
void addt(const emxArray_real_T *y, const emxArray_real_T *X, const
          ...
          const int varargin_4_size[2], double varargin_6, struct0_T *out)
{
  ...
  if (varargin_6 == 1.0) {
    ...
    if (varargin_2) {
      ...
      for (j = 0; j < i; j++) {
        aoffset++;
        E->data[aoffset] = b_varargin_2->data[j];
      }
    }
  ...
  if ((varargin_4_size[0] != 0) && (varargin_4_size[1] != 0)) {
    ...
    /*   if la1==0 */
    b_n = varargin_4_size[0] * varargin_4_size[1];
    for (i = 0; i < b_n; i++) {
    ...
    if (c_y) {
      ...
      for (i = 0; i < b_n; i++) {
        zAw_data[i] = varargin_4_data[i] * b;
      }

      mrdiv(Aw, zAw_data, b_varargin_4_size, z);
    }
  } else {
    i = z->size[0] * z->size[1];
    z->size[0] = b_y->size[0];
  ...
  if (b_w < 1.0E-12) {
    ...
    if (c_y) {
      ...
      tcdf(y_data, y_size, ((double)n - (double)p) - 1.0, zAw_data,
           b_varargin_4_size);
      b_n = b_varargin_4_size[0] * b_varargin_4_size[1];
      for (i = 0; i < b_n; i++) {
