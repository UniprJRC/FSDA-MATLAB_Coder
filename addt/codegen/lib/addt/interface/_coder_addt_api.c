                                        ...
                                        const struct0_T *u)
{
  ...
  emlrtExportNumericArrayR2013b((emlrtCTX)sp, m, &u_data, 8);
  emlrtAssign(&b_y, m);
  emlrtSetFieldR2017b(y, 0, (const char_T *)"b", b_y, 0);
  loop_ub = u->S2add.size[0];
  for (i = 0; i < loop_ub; i++) {
    u_data = u->S2add.data[0];
  }
  c_y = NULL;
  i = u->S2add.size[0];
  m = emlrtCreateNumericArray(1, &i, mxDOUBLE_CLASS, mxCOMPLEX);
  emlrtExportNumericArrayR2013b((emlrtCTX)sp, m, &u_data, 8);
  emlrtAssign(&c_y, m);
  emlrtSetFieldR2017b(y, 0, (const char_T *)"S2add", c_y, 1);
  d_y = NULL;
  iv[0] = u->Tadd.size[0];
  iv[1] = u->Tadd.size[1];
  m = emlrtCreateNumericArray(2, &iv[0], mxDOUBLE_CLASS, mxCOMPLEX);
  emlrtExportNumericArrayR2013b((emlrtCTX)sp, m, &u->Tadd.data[0], 8);
  emlrtAssign(&d_y, m);
  emlrtSetFieldR2017b(y, 0, (const char_T *)"Tadd", d_y, 2);
  e_y = NULL;
  iv[0] = u->pval.size[0];
  iv[1] = u->pval.size[1];
  ...
  i = 0;
  loop_ub = 0;
  while (loop_ub < u->pval.size[1]) {
    loop_ub = 0;
    while (loop_ub < u->pval.size[0]) {
      pData[i] = u->pval.data[0];
      i++;
      loop_ub = 1;
    }
    loop_ub = 1;
  ...
  emlrtAssign(&e_y, m);
  emlrtSetFieldR2017b(y, 0, (const char_T *)"pval", e_y, 3);
  return y;
}
