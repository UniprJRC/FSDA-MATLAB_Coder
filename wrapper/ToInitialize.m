...
for i=1:n
    ...
    if isempty(FileName)
        ...
        for ii = 1:length(d)
            ...
            fileID = fopen(FileWithFullPath, 'w');
            if fileID==-1
            ...
            % Write into fileID
            nb=fprintf(fileID,'%s',fstringNEW);
        ...
        elseif ~isempty(sIndexToreplace)
            error('FSDA:ToInitialize:WrongString',['String ' '''' sIndexToreplace '''' ' already exists in input file ' '''' FileName ''''])
        else
        end
        % Perform search and replace
