  ...
  if (b_ifWhileCond(r)) {
    ...
    if ((varargin_4_size[0] != 0) && (varargin_4_size[1] != 0)) {
      ...
      if (b_ifWhileCond(&nout_data)) {
        ...
        emxInit_real_T(&b, 1);
        i = b->size[0];
        b->size[0] = (int)((double)Y->size[0] - init_contents);
        emxEnsureCapacity_real_T(b, i);
        loop_ub = (int)((double)Y->size[0] - init_contents);
        for (i = 0; i < loop_ub; i++) {
          b->data[i] = 1.0;
        }
        if ((int)((double)Y->size[0] - init_contents) == 0) {
          i = b_bs->size[0];
          b_bs->size[0] = 1;
          emxEnsureCapacity_real_T(b_bs, i);
          b_bs->data[0] = 0.0;
          ...
          b_bs->size[0] = 1;
          emxEnsureCapacity_real_T(b_bs, i);
          cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, (MKL_INT)1,
                      (MKL_INT)1, (MKL_INT)1, 1.0, &varargin_4_data[0],
                      (MKL_INT)1, &b->data[0],
                      (MKL_INT)(int)((double)Y->size[0] - init_contents), 0.0,
                      &b_bs->data[0], (MKL_INT)1);
        }
        emxFree_real_T(&b);
      ...
      irank = 0;
      for (b_i = 0; b_i < 5; b_i++) {
        NoFalseSig = ((int)b_nout_data[2 * b_i] == 9999);
        if (NoFalseSig) {
          trueCount++;
      ...
      for (i = 0; i < trueCount; i++) {
        c_nout_data[i] = ((int)b_nout_data[2 * (tmp_data[i] - 1) + 1] >= 10);
      }
      ...
      if ((varargin_4_size[0] == 0) || (varargin_4_size[1] == 0)) {
        ...
        if (out_mmd->data[(int)ii - 1] < tr - 1.0) {
          ...
          if (b_loop_ub > 0) {
            ...
            d = gfind->data[r7->data[0] - 1] - out_mmd->data[0];
            emxFree_int32_T(&r7);
            if (b_i > d) {
              i = 0;
              i1 = 0;
    ...
    loop_ub = Y->size[1];
    i = out_cov->size[0] * out_cov->size[1];
    out_cov->size[0] = goodobs->size[1];
    out_cov->size[1] = Y->size[1];
    emxEnsureCapacity_real_T(out_cov, i);
    emxFree_int32_T(&ia);
    for (i = 0; i < loop_ub; i++) {
      b_loop_ub = goodobs->size[1];
      for (i1 = 0; i1 < b_loop_ub; i1++) {
        out_cov->data[i1 + out_cov->size[0] * i] =
            Y->data[((int)goodobs->data[i1] + Y->size[0] * i) - 1];
      }
    }
    combineVectorElements(out_cov, loc);
    i = loc->size[0] * loc->size[1];
    loc->size[0] = 1;
    emxEnsureCapacity_real_T(loc, i);
    loop_ub = loc->size[1] - 1;
    for (i = 0; i <= loop_ub; i++) {
      loc->data[i] /= (double)goodobs->size[1];
    }
    loop_ub = Y->size[1];
    i = Ytilde->size[0] * Ytilde->size[1];
    Ytilde->size[0] = goodobs->size[1];
    Ytilde->size[1] = Y->size[1];
    emxEnsureCapacity_real_T(Ytilde, i);
    for (i = 0; i < loop_ub; i++) {
      b_loop_ub = goodobs->size[1];
    ...
    for (i = 0; i < loop_ub; i++) {
      r6->data[i] = r5->data[i] * Ytilde->data[i];
    }
    emxFree_real_T(&r5);
    sum(r6, bs);
