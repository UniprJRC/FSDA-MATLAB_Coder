...
void FSM_wrapper(const emxArray_real_T *Y, const double bonflev_data[],
                 ...
                 bool msg, bool nocheck, double rf, struct0_T *out)
{
  ...
  if (b_ifWhileCond(r)) {
    ...
    if ((bonflev_size[0] != 0) && (bonflev_size[1] != 0)) {
      ...
      if (b_ifWhileCond(&nout_data)) {
        ...
        emxInit_real_T(&b, 1);
        i = b->size[0];
        b->size[0] = (int)((double)b_Y->size[0] - init_contents);
        emxEnsureCapacity_real_T(b, i);
        loop_ub = (int)((double)b_Y->size[0] - init_contents);
        for (i = 0; i < loop_ub; i++) {
          b->data[i] = 1.0;
        }
        if ((int)((double)b_Y->size[0] - init_contents) == 0) {
          i = c_m0->size[0];
          c_m0->size[0] = 1;
          emxEnsureCapacity_real_T(c_m0, i);
          c_m0->data[0] = 0.0;
          ...
          cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, (MKL_INT)1,
                      (MKL_INT)1, (MKL_INT)1, 1.0, &bonflev_data[0], (MKL_INT)1,
                      &b->data[0],
                      (MKL_INT)(int)((double)b_Y->size[0] - init_contents), 0.0,
                      &c_m0->data[0], (MKL_INT)1);
        }
        emxFree_real_T(&b);
    ...
    loop_ub = b_Y->size[1];
    i = bb->size[0] * bb->size[1];
    bb->size[0] = goodobs->size[1];
    bb->size[1] = b_Y->size[1];
    emxEnsureCapacity_real_T(bb, i);
    emxFree_int32_T(&ia);
    for (i = 0; i < loop_ub; i++) {
      b_loop_ub = goodobs->size[1];
      for (i1 = 0; i1 < b_loop_ub; i1++) {
        bb->data[i1 + bb->size[0] * i] =
            b_Y->data[((int)goodobs->data[i1] + b_Y->size[0] * i) - 1];
      }
    }
    combineVectorElements(bb, loc);
    i = loc->size[0] * loc->size[1];
    loc->size[0] = 1;
    emxEnsureCapacity_real_T(loc, i);
