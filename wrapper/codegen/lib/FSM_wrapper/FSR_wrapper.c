...
void FSR_wrapper(const emxArray_real_T *y, const emxArray_real_T *X,
                 ...
                 const int threshoutX_size[2], bool weak, struct1_T *out)
{
  ...
  if (rtIsNaN(n + 1.0) || rtIsNaN(bonflevoutX_data)) {
    ...
    i = d_y->size[0] * d_y->size[1];
    d_y->size[0] = 1;
    loop_ub = (int)floor(bonflevoutX_data - (n + 1.0));
  ...
  if (n <= 5000.0) {
    ...
    if (rtIsNaN(bonflevoutX_data)) {
      ...
      i = iniseq->size[0] * iniseq->size[1];
      iniseq->size[0] = 1;
      loop_ub = (int)floor((bonflevoutX_data - 100.0) / 100.0);
  ...
  emxEnsureCapacity_real_T(b_expl_temp, i);
  loop_ub--;
  for (i = 0; i <= loop_ub; i++) {
  ...
  INP_mdr->size[1] = 2;
  emxEnsureCapacity_real_T(INP_mdr, i);
  ...
  INP_Un->size[1] = 11;
  emxEnsureCapacity_real_T(INP_Un, i);
  ...
  if ((b_X->size[0] == 0) || (b_X->size[1] == 0) ||
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                (MKL_INT)b_X->size[0], (MKL_INT)1, (MKL_INT)b_X->size[1], 1.0,
                &b_X->data[0], (MKL_INT)b_X->size[0],
                &c_expl_temp.beta->data[0], (MKL_INT)c_expl_temp.beta->size[0],
                0.0, &constr->data[0], (MKL_INT)b_X->size[0]);
  }
  emxFree_real_T(&b_X);
