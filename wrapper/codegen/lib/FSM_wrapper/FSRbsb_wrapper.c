                    ...
                    emxArray_real_T *Un, emxArray_real32_T *BB)
{
  ...
  if (guard1) {
    ...
    for (mm = 0; mm < i; mm++) {
      ...
      loop_ub = b_X->size[0];
      if ((b_X->size[0] == 0) || (b_X->size[1] == 0) || (blast->size[0] == 0)) {
        i1 = b_a->size[0];
        b_a->size[0] = b_X->size[0];
        ...
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                    (MKL_INT)b_X->size[0], (MKL_INT)1, (MKL_INT)b_X->size[1],
                    1.0, &b_X->data[0], (MKL_INT)b_X->size[0], &blast->data[0],
                    (MKL_INT)blast->size[0], 0.0, &b_a->data[0],
                    (MKL_INT)b_X->size[0]);
      }
      loop_ub = b_y->size[0];
      i1 = b_a->size[0];
