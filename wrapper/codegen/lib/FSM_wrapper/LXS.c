         ...
         struct8_T *out)
{
  ...
  if ((varargin_12_size[0] != 0) && (varargin_12_size[1] != 0)) {
    ...
    nx = outFSM_outliers->size[0] * outFSM_outliers->size[1];
    emxFree_real_T(&c_X);
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    if (guard1) {
      ...
      for (i1 = 0; i1 < loop_ub; i1++) {
        for (i2 = 0; i2 < nx; i2++) {
          Xb->data[i2 + Xb->size[0] * i1] =
              X->data[((int)C->data[b_i + C->size[0] * i2] + X->size[0] * i1) -
                      1];
        }
      }
      ...
      for (i1 = 0; i1 < nx; i1++) {
        r2->data[i1] = y->data[(int)C->data[b_i + C->size[0] * i1] - 1];
      }
      mldivide(Xb, r2, b);
    }
    if ((!rtIsNaN(b->data[0])) && (!rtIsInf(b->data[0]))) {
      ...
      if ((X->size[0] == 0) || (X->size[1] == 0) || (b->size[0] == 0)) {
        ...
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                    ...
                    (MKL_INT)b->size[0], 0.0, &out->residuals->data[0],
                    (MKL_INT)X->size[0]);
      }
      ...
      emxEnsureCapacity_real_T(out->residuals, i1);
      for (i1 = 0; i1 < nx; i1++) {
        out->residuals->data[i1] = y->data[i1] - out->residuals->data[i1];
      }
      i1 = r2->size[0];
  ...
  if ((X->size[0] == 0) || (X->size[1] == 0) || (out->beta->size[0] == 0)) {
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, (MKL_INT)X->size[0],
                ...
                (MKL_INT)X->size[0], &out->beta->data[0],
                (MKL_INT)out->beta->size[0], 0.0, &out->residuals->data[0],
                (MKL_INT)X->size[0]);
  }
  i = out->residuals->size[0];
  out->residuals->size[0] = y->size[0];
                ...
                double *outIRWLS_numscale2rw, emxArray_real_T *outIRWLS_weights)
{
  ...
  if ((X->size[0] == 0) || (X->size[1] == 0) || (initialbeta->size[0] == 0)) {
    i = beta->size[0];
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, (MKL_INT)X->size[0],
                (MKL_INT)1, (MKL_INT)X->size[1], 1.0, &X->data[0],
                (MKL_INT)X->size[0], &initialbeta->data[0],
                (MKL_INT)initialbeta->size[0], 0.0, &beta->data[0],
                (MKL_INT)X->size[0]);
  }
  emxInit_real_T(&r2, 1);
  /*  Squared residuals for all the observations */
  ...
  while ((!exitg1) && ((betadiff > reftol) && (iter < refsteps))) {
    ...
    for (i = 0; i < nx; i++) {
      for (k = 0; k < loop_ub_tmp; k++) {
        b_X->data[k + b_X->size[0] * i] =
            X->data[(i_r2s->data[k] + X->size[0] * i) - 1];
      }
    }
    i = r2->size[0];
    r2->size[0] = (int)h;
    ...
    if (b_y) {
      ...
      *outIRWLS_numscale2rw = initialscale;
      ...
      if ((X->size[0] == 0) || (X->size[1] == 0) ||
        ...
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                    (MKL_INT)X->size[0], (MKL_INT)1, (MKL_INT)X->size[1], 1.0,
                    &X->data[0], (MKL_INT)X->size[0], &outIRWLS_betarw->data[0],
                    (MKL_INT)outIRWLS_betarw->size[0], 0.0, &beta->data[0],
                    (MKL_INT)X->size[0]);
      }
           ...
           const emxArray_real_T *varargin_6, bool varargin_10, struct8_T *out)
{
  ...
  if ((nsamp->size[0] == 1) && (nsamp->size[1] == 1)) {
    ...
    nx = nsamp->size[0] * nsamp->size[1] - 1;
    for (i = 0; i <= nx; i++) {
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    for (i1 = 0; i1 < k; i1++) {
      for (i2 = 0; i2 < nx; i2++) {
        Xb->data[i2 + Xb->size[0] * i1] =
            X->data[((int)nsamp->data[b_i + nsamp->size[0] * i2] +
                     X->size[0] * i1) -
                    1];
      }
    }
    ...
    emxEnsureCapacity_real_T(r2, i1);
    for (i1 = 0; i1 < nx; i1++) {
      r2->data[i1] = y->data[(int)nsamp->data[b_i + nsamp->size[0] * i1] - 1];
    }
    mldivide(Xb, r2, b);
    if ((!rtIsNaN(b->data[0])) && (!rtIsInf(b->data[0]))) {
      /*  Residuals for all observations using b based on subset */
      ...
      nx = varargin_2->size[1];
      for (i1 = 0; i1 < nx; i1++) {
        x->data[i1] = (varargin_2->data[i1] == 1.0);
      }
      c_y = (x->size[1] != 0);
      if (c_y) {
        k = 0;
