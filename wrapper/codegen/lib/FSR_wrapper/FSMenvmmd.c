...
void FSMenvmmd(double n, double v, double varargin_4, emxArray_real_T *MMDenv)
{
  ...
  k = b_tmp->size[0] * b_tmp->size[1];
  b_tmp->size[0] = x->size[0];
  b_tmp->size[1] = 6;
  emxEnsureCapacity_real_T(b_tmp, k);
  nrows = x->size[0] * 6;
  for (k = 0; k < nrows; k++) {
    b_tmp->data[k] = x->data[k] / b_y->data[k] * r2->data[k];
  }
  nrows = b_tmp->size[0] * 6;
  for (k = 0; k < nrows; k++) {
  ...
  nrows = x->size[0] * 6;
  for (k = 0; k < nrows; k++) {
    x->data[k] = x->data[k] / b_y->data[k] * r2->data[k];
  }
  emxFree_real_T(&b_y);
  emxFree_real_T(&r2);
  nrows = x->size[0] * 6;
  ...
  nrows = x->size[0] * 6;
  for (k = 0; k < nrows; k++) {
    b_tmp->data[k] = x->data[k] / b->data[k];
  }
  emxFree_real_T(&b);
  emxFree_real_T(&x);
...
void b_FSMenvmmd(double n, double v, double varargin_4, emxArray_real_T *MMDenv)
{
  ...
  nrows = b_tmp->size[0];
  for (k = 0; k < nrows; k++) {
    b_y->data[k] = 2.0 * b_tmp->data[k];
  }
  emxInit_real_T(&quant, 2);
  emxInit_real_T(&b_x, 2);
  b_finv(b_b, x, b_y, b_x);
  k = quant->size[0] * quant->size[1];
  ...
  nrows = x->size[0];
  for (k = 0; k < nrows; k++) {
    b_x->data[k] = x->data[k] / b_y->data[k] * r->data[k];
  }
  nrows = b_x->size[0];
  for (k = 0; k < nrows; k++) {
  ...
  nrows = x->size[0];
  for (k = 0; k < nrows; k++) {
    x->data[k] = x->data[k] / b_y->data[k] * r->data[k];
  }
  emxFree_real_T(&b_y);
  emxFree_real_T(&r);
  nrows = x->size[0];
  ...
  for (k = 0; k < outsize_idx_1; k++) {
    for (i = 0; i < itilerow; i++) {
      MMDenv->data[i + MMDenv->size[0] * input_sizes_idx_1] = b_x->data[i];
    }
  }
  emxFree_real_T(&b_x);
}
...
void c_FSMenvmmd(double n, double v, double varargin_4, emxArray_real_T *MMDenv)
{
  ...
  emxFree_real_T(&quant);
  for (k = 0; k < nrows; k++) {
    b->data[k] = n / b->data[k] * c_b_tmp->data[k];
  }
  ...
  nrows = x->size[0] * 4;
  for (k = 0; k < nrows; k++) {
    b_b_tmp->data[k] = x->data[k] / b_y->data[k] * b_tmp->data[k];
  }
  ...
  nrows = x->size[0] * 4;
  for (k = 0; k < nrows; k++) {
    b_b_tmp->data[k] = x->data[k] / b->data[k];
  }
  emxFree_real_T(&b);
  emxFree_real_T(&x);
  k = MMDenv->size[0] * MMDenv->size[1];
