...
void FSRenvmdr(double n, double p, double varargin_4, emxArray_real_T *MDRenv)
{
  ...
  for (i = 0; i < nrows; i++) {
    b_mm->data[i] = MinSca->data[i] / mm->data[i];
  }
...
void b_FSRenvmdr(double n, double p, double varargin_4, emxArray_real_T *MDRenv)
{
  ...
  nrows = b->size[0];
  for (i = 0; i < nrows; i++) {
    b_b_tmp->data[i] = 0.5 * (b->data[i] / n + 1.0);
  }
  i = x->size[0] * x->size[1];
  ...
  for (outsize_idx_1 = 0; outsize_idx_1 < i; outsize_idx_1++) {
    if ((b_b_tmp->data[outsize_idx_1] >= 0.0) &&
        (b_b_tmp->data[outsize_idx_1] <= 1.0)) {
      x->data[outsize_idx_1] =
          -1.4142135623730951 * erfcinv(2.0 * b_b_tmp->data[outsize_idx_1]);
    } else {
      x->data[outsize_idx_1] = rtNaN;
    }
  }
  /* corr=1-(2*a.*normpdf(a))./(2*normcdf(a)-1); */
  i = b_b_tmp->size[0] * b_b_tmp->size[1];
  ...
  i = x->size[0];
  for (outsize_idx_1 = 0; outsize_idx_1 < i; outsize_idx_1++) {
    b_b_tmp->data[outsize_idx_1] =
        exp(-0.5 * x->data[outsize_idx_1] * x->data[outsize_idx_1]) /
        2.5066282746310002;
  }
  nrows = b->size[0];
  for (i = 0; i < nrows; i++) {
    b->data[i] = 1.0 - 2.0 * (n / b->data[i]) * x->data[i] * b_b_tmp->data[i];
  }
  emxFree_real_T(&x);
