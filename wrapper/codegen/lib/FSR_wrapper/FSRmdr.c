            ...
            emxArray_real_T *Bols, emxArray_real_T *S2)
{
  ...
  for (mm = 0; mm <= i; mm++) {
    ...
    if (varargin_8 && (n > 5000)) {
      d_do_vectors(b_mm, b_y, (double *)&thpmm_data, c_size,
    ...
    if (b_mm >= init1) {
      ...
      if ((int)b_mm < n) {
        ...
        r3->size[0] = Xncl->size[0];
        r3->size[1] = Xncl->size[1];
        emxEnsureCapacity_real_T(r3, i1);
        loop_ub = Xncl->size[0] * Xncl->size[1];
        for (i1 = 0; i1 < loop_ub; i1++) {
          r3->data[i1] = Xncl->data[i1];
        }
        c_mtimes(Xb, Xb, r4);
        mrdiv(r3, r4);
