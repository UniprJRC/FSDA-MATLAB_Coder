...
#include "FSRbsb_wrapper_internal_types.h"
                    ...
                    emxArray_real_T *Un, emxArray_real32_T *BB)
{
  ...
  bool Ra;
  bool guard1 = false;
  if (!isInitialized_FSRbsb_wrapper) {
  ...
  if ((bsbsteps->size[0] == 0) || (bsbsteps->size[1] == 0)) {
    ...
    if (n <= 5000.0) {
      if (rtIsNaN(b_init)) {
        ...
        if ((rtIsInf(b_init) || rtIsInf(n)) && (b_init == n)) {
          irank = 1;
        } else if (floor(b_init) == b_init) {
          irank = (int)floor(n - b_init) + 1;
        } else {
      ...
      if (rtIsNaN(a) || rtIsNaN(b)) {
        ...
        emxEnsureCapacity_real_T(d_y, i);
        for (i = 0; i <= loop_ub; i++) {
          d_y->data[i] = a + 100.0 * (double)i;
        }
      } else {
        b_eml_float_colon(a, b, d_y);
      }
      i = b_bsbsteps->size[0];
      b_bsbsteps->size[0] = d_y->size[1] + 1;
      emxEnsureCapacity_real_T(b_bsbsteps, i);
      b_bsbsteps->data[0] = b_init;
      loop_ub = d_y->size[1];
      for (i = 0; i < loop_ub; i++) {
        b_bsbsteps->data[i + 1] = d_y->data[i];
      }
      emxFree_real_T(&d_y);
    }
    ...
    for (mm = 0; mm < i; mm++) {
      ...
      if (b_mm < n) {
        ...
        if (b_mm >= b_init) {
          ...
          /*                  else */
          /*                      disp(['Warning: interchange greater than 10
           * when m=' int2str(mm)]); */
