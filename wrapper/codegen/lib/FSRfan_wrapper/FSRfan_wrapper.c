                    ...
                    struct0_T *out)
{
  ...
  bool guard1 = false;
  if (!isInitialized_FSRfan_wrapper) {
  ...
  i = out->Un->size[0];
  out->Un->size[0] = b_n;
  emxEnsureCapacity_cell_wrap_0(out->Un, i);
  emxInit_real_T(&z, 1);
  ...
  for (b_i = 0; b_i < b_n; b_i++) {
    ...
    loop_ub = lms->size[0];
    if (lms->size[0] == 1) {
      LXS(z, out->X, lms, h, nsampArray, msg, &expl_temp);
      i = bsb->size[0];
