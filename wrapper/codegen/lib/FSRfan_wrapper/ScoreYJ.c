             ...
             int *outSC_Score_size, double *outSC_Lik)
{
  ...
  logG = sum(vposlai) / (double)X->size[0];
  end = y->size[0] - 1;
  nx = 0;
  for (i = 0; i <= end; i++) {
    if (!(y->data[i] >= 0.0)) {
  ...
  *outSC_Score_size = 1;
  ...
  if (fabs(varargin_2 - 2.0) > 1.0E-8) {
    ...
    nx = vposlai->size[0];
    for (i = 0; i < nx; i++) {
      znonnegs->data[i] = (1.0 - vposlai->data[i]) / Glaminus1;
    }
    end = y->size[0];
    nx = 0;
    for (i = 0; i < end; i++) {
    ...
    for (i = 0; i < nx; i++) {
      vposlai->data[i] =
          (vposlai->data[i] * (vneg->data[i] + b_k) - b_k) / Glaminus1;
    }
    end = y->size[0];
    nx = 0;
    for (i = 0; i < end; i++) {
  ...
  for (i = 0; i < nx; i++) {
    for (k = 0; k < end; k++) {
      Xw->data[k + Xw->size[0] * i] = X->data[k + end * i];
    }
  }
  loop_ub = b_i;
  for (i = 0; i < loop_ub; i++) {
    for (k = 0; k < end; k++) {
      Xw->data[k + Xw->size[0] * nx] = w->data[k];
    }
  }
  /*  New code */
  i = vposlai->size[0];
  ...
  for (k = 0; k < loop_ub; k++) {
    nx = k * Xw->size[0];
    for (i = 0; i <= end; i++) {
      w->data[i] += Xw->data[nx + i] * vposlai->data[k];
    }
  }
  emxFree_real_T(&Xw);
  i = w->size[0];
  ...
  nx = z->size[0];
  for (i = 0; i < nx; i++) {
    w->data[i] = z->data[i] - w->data[i];
  }
  G = rt_powd_snf(b_norm(w), 2.0);
  ...
  for (k = 0; k < inner; k++) {
    nx = k * X->size[0];
    for (i = 0; i <= mc; i++) {
      w->data[i] += X->data[nx + i] * znonnegs->data[k];
    }
  }
  ...
  emxFree_real_T(&w);
  outSC_Score_data[0] =
      -Glaminus1 * sqrt((rt_powd_snf(b_norm(z), 2.0) - G) /
                        (G / ((double)(X->size[0] - X->size[1]) - 1.0)));
