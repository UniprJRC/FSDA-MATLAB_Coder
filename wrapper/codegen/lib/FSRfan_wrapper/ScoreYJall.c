                ...
                int outSC_Score_size[2])
{
  ...
  logGpos = sum(logvpos) / (double)X->size[0];
  i = logvneg->size[0];
  ...
  for (i = 0; i < loop_ub; i++) {
    vposlai->data[i] = -logvneg->data[i];
  }
  logGneg = sum(vposlai) / (double)X->size[0];
  G = exp(logG);
  /* Gpos=exp(logGpos); */
  /* Gneg=exp(logGneg); */
  ...
  if (fabs(varargin_2) > 1.0E-8) {
    ...
    loop_ub = vposlai->size[0];
    for (i = 0; i < loop_ub; i++) {
      z->data[i] = (vposlai->data[i] * (logvpos->data[i] - kpos) + kpos) / q;
    }
    k = y->size[0];
    nx = 0;
    for (loop_ub = 0; loop_ub < k; loop_ub++) {
      if (y->data[loop_ub] >= 0.0) {
        vpos->data[loop_ub] = z->data[nx];
        nx++;
      }
    }
    ...
    loop_ub = znonnegs->size[0];
    for (i = 0; i < loop_ub; i++) {
      vposlai->data[i] = znonnegs->data[i] * (z->data[i] - logGpos);
    }
    k = y->size[0];
    nx = 0;
    for (loop_ub = 0; loop_ub < k; loop_ub++) {
  ...
  if (fabs(2.0 - varargin_2) > 1.0E-8) {
    ...
    loop_ub = znonnegs->size[0];
    for (i = 0; i < loop_ub; i++) {
      z->data[i] = (znonnegs->data[i] * (logvneg->data[i] + kpos) - kpos) / q;
    }
    k = y->size[0];
    nx = 0;
    for (loop_ub = 0; loop_ub < k; loop_ub++) {
      if (!(y->data[loop_ub] >= 0.0)) {
        wneg->data[loop_ub] = z->data[nx];
        nx++;
      }
    }
    kpos = logG - Glaminus1;
    ...
    emxEnsureCapacity_real_T(z, i);
    for (i = 0; i < loop_ub; i++) {
      z->data[i] = logvneg->data[i] * (z->data[i] + logG) / G;
    }
    k = y->size[0];
    nx = 0;
    for (loop_ub = 0; loop_ub < k; loop_ub++) {
      if (!(y->data[loop_ub] >= 0.0)) {
        w->data[loop_ub] = z->data[nx];
        nx++;
      }
    }
  }
  ...
  loop_ub = vneg->size[0];
  for (i = 0; i < loop_ub; i++) {
    vposlai->data[i] = vneg->data[i] - vposlai->data[i];
  }
  q = rt_powd_snf(b_norm(vposlai), 2.0);
  ...
  loop_ub = vneg->size[0];
  for (i = 0; i < loop_ub; i++) {
    z->data[i] = vneg->data[i] - z->data[i];
  }
  kpos = rt_powd_snf(b_norm(z), 2.0);
  ...
  if (vposboo && vnegboo) {
    ...
    kpos = rt_powd_snf(b_norm(vneg), 2.0);
    outSC_Score_data[3] =
    ...
    outSC_Score_data[3] = rt_powd_snf(outSC_Score_data[1], 2.0);
  } else {
    /*  in this case there are just negative observations */
    outSC_Score_data[3] = rt_powd_snf(kpos, 2.0);
  }
  emxFree_real_T(&z);
  emxFree_real_T(&Xw);
