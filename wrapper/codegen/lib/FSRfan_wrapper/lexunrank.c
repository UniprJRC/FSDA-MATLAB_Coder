...
void b_lexunrank(double n, double k, double N, emxArray_real_T *kcomb)
{
  ...
  for (kk = 0; kk < i; kk++) {
    ...
    if (b_kk == k) {
      ...
      for (x_data = 0; x_data < i1; x_data++) {
        maxx = b_kk + (double)x_data;
      ...
      for (i3 = 0; i3 < x_data; i3++) {
        y->data[i3] = pas_col->data[(i1 + i3) - 1] * (b_kk + 1.0) /
                      (seq->data[(i2 + i3) - 1] - b_kk);
      }
      x_data = y->size[1];
      for (i1 = 0; i1 < x_data; i1++) {
        pas_col->data[(x_size + i1) + 1] = y->data[i1];
      }
    }
    ...
    x_data = i2 - i1;
    i2 = b_pas_col->size[0];
    b_pas_col->size[0] = x_data;
    emxEnsureCapacity_boolean_T(b_pas_col, i2);
    for (i2 = 0; i2 < x_data; i2++) {
      b_pas_col->data[i2] = (pas_col->data[i1 + i2] > N_kk);
    }
    eml_find(b_pas_col, (int *)&x_data, &x_size);
    if (x_size == 0) {
      maxx = n - 1.0;
    } else {
      maxx = ((double)x_data + b_kk) - 2.0;
    }
               ...
               emxArray_real_T *kcomb)
{
  ...
  for (kk = 0; kk < i; kk++) {
    ...
    pascalM_size = x_data;
    for (x_size = 0; x_size < x_data; x_size++) {
      b_pascalM_data[x_size] =
          (pascalM->data[x_size + pascalM->size[0] * ((int)(b_kk + 1.0) - 1)] >
           N_kk);
    }
    ...
    eml_find(&pascalM_data, (int *)&x_data, &x_size);
    /*  seqnmkk=1:n-kk; */
