...
#include "FSRmdr_wrapper_internal_types.h"
                    ...
                    emxArray_real_T *Bols, emxArray_real_T *S2)
{
  ...
  bool guard1 = false;
  bool guard2 = false;
  if (!isInitialized_FSRmdr_wrapper) {
  ...
  if (rtIsNaN(init1) || rtIsNaN(n)) {
    ...
    emxEnsureCapacity_real_T(c_y, i);
    for (i = 0; i <= loop_ub; i++) {
      c_y->data[i] = init1 + (double)i;
    }
  } else {
    eml_float_colon(init1, n, c_y);
  }
  ...
  if (rtIsNaN(init1) || rtIsNaN(n)) {
    ...
    i = c_y->size[0] * c_y->size[1];
    c_y->size[0] = 1;
    c_y->size[1] = (int)floor(n - init1) + 1;
    emxEnsureCapacity_real_T(c_y, i);
    loop_ub = (int)floor(n - init1);
    for (i = 0; i <= loop_ub; i++) {
      c_y->data[i] = init1 + (double)i;
  ...
  if (rtIsNaN(init1) || rtIsNaN(n - 1.0)) {
    ...
    emxEnsureCapacity_real_T(c_y, i1);
    for (i1 = 0; i1 <= loop_ub; i1++) {
      c_y->data[i1] = init1 + (double)i1;
    }
  } else {
    eml_float_colon(init1, n - 1.0, c_y);
  }
  loop_ub = (int)(n - init1);
  i1 = bsbx->size[0];
  bsbx->size[0] = loop_ub;
  ...
  mdr->size[0] = c_y->size[1];
  mdr->size[1] = 2;
  emxEnsureCapacity_real_T(mdr, i1);
  irank = c_y->size[1];
  for (i1 = 0; i1 < irank; i1++) {
    mdr->data[i1] = c_y->data[i1];
  }
  irank = bsbx->size[0];
  for (i1 = 0; i1 < irank; i1++) {
  ...
  if (b_ifWhileCond(c_bsbsteps)) {
    ...
    irank = c_y->size[1];
    for (i1 = 0; i1 < irank; i1++) {
      b_bsbsteps->data[i1] = c_y->data[i1];
    }
  ...
  if (rtIsNaN(init1 + 1.0) || rtIsNaN(n)) {
    ...
    emxEnsureCapacity_real_T(c_y, i);
    for (i = 0; i <= irank; i++) {
      c_y->data[i] = (init1 + 1.0) + (double)i;
    }
  } else {
    eml_float_colon(init1 + 1.0, n, c_y);
  }
  i = e->size[0];
  e->size[0] = c_y->size[1];
  emxEnsureCapacity_real_T(e, i);
  ...
  if (guard1) {
    ...
    do {
      ...
      if (mm <= (int)(n + (1.0 - (double)ini0)) - 1) {
        ...
        if (msg && (n > 5000.0)) {
          do_vectors(b_mm, seq500, (double *)&initdef, c_size, (int *)&nwhile,
          ...
          while (nwhile == 1) {
            ...
            while ((!exitg2) && (b_i <= ncl->size[0] - 1)) {
              ...
              if (irank == p) {
                ...
                b_do_vectors(seq, e, ncl, ia, &result);
                nwhile = 1;
                exitg2 = true;
              }
            }
            ...
            if (Ra) {
              ...
              if (b_mm < n) {
                ...
                if (bonflevout) {
                  ...
                  if ((0 <= irank) && Ra) {
                    truerownamestopenalize_data = (unsigned int)ncl->data[0];
                  }
                  /*  disp(ncl(unittopenalize)) */
                  for (i = 0; i < trueCount; i++) {
                    initdef = r->data[((int)truerownamestopenalize_data +
                                       r->size[0]) -
                                      1] *
                              1.0E+7;
                  }
                  for (i = 0; i < trueCount; i++) {
                    r->data[((int)truerownamestopenalize_data + r->size[0]) -
                            1] = initdef;
                  }
                }
                ...
                if (initdef == 0.0) {
                  ...
                  mdr->data[i1 + mdr->size[0]] = sqrt(minimum(e) / initdef);
                }
              }
              /* if mm<n */
            }
          ...
          if (b_mm < n) {
            ...
            if (b_mm < n - 1.0) {
              if ((constr->size[0] != 0) && (constr->size[1] != 0) &&
                ...
                b_do_vectors(bsbx, &b_constr, ncl, ia, &result);
              } else {
                if (b_mm + 2.0 > n) {
                  i = 0;
                  i1 = 0;
