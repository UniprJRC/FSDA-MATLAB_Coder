...
void FSRinvmdr(const double mdr[2], double p, double MDRinv[3])
{
  ...
  a = norminv(0.75);
  MDRncoord =
      mdr[1] * sqrt(1.0 - 4.0 * a * (exp(-0.5 * a * a) / 2.5066282746310002));
  if ((1.0 - p > 0.0) && (!rtIsNaN(MDRncoord))) {
    if (MDRncoord == 0.0) {
      a = 0.5;
    } else if (1.0 - p == 1.0) {
      a = atan(1.0 / -MDRncoord) / 3.1415926535897931;
      if (MDRncoord > 0.0) {
        a++;
      }
    } else {
  ...
  if (!rtIsNaN(MDRncoord)) {
    if (MDRncoord > 0.0) {
      if (rtIsInf(MDRncoord)) {
        a = 1.0;
      } else if (4.0 <= MDRncoord * 2.0) {
        a = (b_betainc(4.0 / (MDRncoord * 2.0 + 4.0), 2.0, 1.0)).re;
      } else {
        a = 2.0 * MDRncoord;
