...
void LTSts_wrapper(const emxArray_real_T *y, double conflev, bool dispresults,
                   ...
                   double SmallSampleCor, bool yxsave, struct3_T *out,
                   emxArray_real_T *C)
{
  ...
  bool b_varargin_22_data[2];
  bool empty_non_axis_sizes;
  bool exitg2;
  (void)nocheck;
  if (!isInitialized_LTSts_wrapper) {
  ...
  do_vectors(model->trend, (double *)&ncomb, c_size, (int *)&nx, &sizes_idx_1,
             (int *)&vlen, &b_loop_ub);
  ...
  for (loop_ub = 0; loop_ub < nx; loop_ub++) {
    b_y->data[loop_ub] = seq.contents->data[loop_ub] * seq.contents->
      data[loop_ub];
  }
  ...
  nx = seq.contents->size[0];
  for (loop_ub = 0; loop_ub < nx; loop_ub++) {
    betaini->data[loop_ub] = rt_powd_snf(seq.contents->data[loop_ub], 3.0);
  }
  ...
  if (intercept) {
    ...
    loop_ub = Seq.contents->size[0];
    nx = Xtrend.contents->size[0] * Xtrend.contents->size[1];
    Xtrend.contents->size[0] = Seq.contents->size[0];
    b_loop_ub = i1 - i;
    Xtrend.contents->size[1] = b_loop_ub;
    emxEnsureCapacity_real_T(Xtrend.contents, nx);
    for (i1 = 0; i1 < b_loop_ub; i1++) {
      for (nx = 0; nx < loop_ub; nx++) {
        Xtrend.contents->data[nx + Xtrend.contents->size[0] * i1] =
          Seq.contents->data[nx + Seq.contents->size[0] * (i + i1)];
      }
    }
  }
  ...
  if (model->seasonal > 0.0) {
    ...
    b_sprintf(model->seasonal, sstring);
    ...
    for (sizes_idx_1 = 0; sizes_idx_1 < i; sizes_idx_1++) {
      ...
      for (loop_ub = 0; loop_ub < nx; loop_ub++) {
        bestnumscale2->data[loop_ub] = cos(bestnumscale2->data[loop_ub]);
      }
      ...
      nx = betaini->size[0];
      for (loop_ub = 0; loop_ub < nx; loop_ub++) {
        betaini->data[loop_ub] = sin(betaini->data[loop_ub]);
      }
    ...
    if (seasonal.contents == model->s / 2.0) {
      ...
      vlen = Xseaso.contents->size[0] - 1;
      b_loop_ub = Xseaso.contents->size[0];
      for (i = 0; i < loop_ub; i++) {
        for (i1 = 0; i1 < b_loop_ub; i1++) {
          Xseaso.contents->data[i1 + (vlen + 1) * i] = Xseaso.contents->data[i1
            + Xseaso.contents->size[0] * i];
        }
      }

      i = Xseaso.contents->size[0] * Xseaso.contents->size[1];
      Xseaso.contents->size[0] = vlen + 1;
  ...
  if (ARp > 0.0) {
    ...
    for (sizes_idx_1 = 0; sizes_idx_1 < i1; sizes_idx_1++) {
      i = (int)((double)y->size[0] - ((double)sizes_idx_1 + 1.0));
      if (1 > i) {
        loop_ub = 0;
      } else {
        loop_ub = i;
      ...
      for (i = 0; i <= sizes_idx_1; i++) {
        Ylagged->data[i + Ylagged->size[0] * sizes_idx_1] = y->data[i];
      }

      for (i = 0; i < loop_ub; i++) {
        Ylagged->data[((i + sizes_idx_1) + Ylagged->size[0] * sizes_idx_1) + 1] =
          y->data[i];
      }
    }
  ...
  if (seasonal.contents < 6.0) {
    ncomb = (model->trend + 1.0) + seasonal.contents * 2.0;
    if (rtIsNaN(model->trend + 2.0) || rtIsNaN(ncomb)) {
    ...
    } else if (ncomb < model->trend + 2.0) {
      indlinsc.contents->size[0] = 1;
      indlinsc.contents->size[1] = 0;
    } else if ((rtIsInf(model->trend + 2.0) || rtIsInf(ncomb)) && (model->trend
                + 2.0 == ncomb)) {
      i = indlinsc.contents->size[0] * indlinsc.contents->size[1];
      indlinsc.contents->size[0] = 1;
      indlinsc.contents->size[1] = 1;
      ...
      i = indlinsc.contents->size[0] * indlinsc.contents->size[1];
      indlinsc.contents->size[0] = 1;
      loop_ub = (int)floor(ncomb - (model->trend + 2.0));
      ...
      eml_float_colon(model->trend + 2.0, ncomb, indlinsc.contents);
    }
  } else {
    ncomb = ((model->trend + 1.0) + seasonal.contents * 2.0) - 1.0;
    if (rtIsNaN(model->trend + 2.0) || rtIsNaN(ncomb)) {
      i = indlinsc.contents->size[0] * indlinsc.contents->size[1];
      indlinsc.contents->size[0] = 1;
      indlinsc.contents->size[1] = 1;
  ...
  b_do_vectors(Cr, indlinsc.contents, otherind.contents, ia, &b_loop_ub);
  ...
  if (ifWhileCond(b_varargin_22_data, b_varargin_22_size)) {
    varargin_22_size[0] = 1;
    varargin_22_size[1] = 1;
  ...
  /*  normal distribution. */
  a = norminv(0.5 * (b_h / (double)y->size[0] + 1.0));
  ...
  if (lshift > 0.0) {
    subsets(varargin_22_data, varargin_22_size, (double)y->size[0] - 1.0, pini +
            1.0, 0.0, C, &a);
    if ((int)a != 0) {
      result = (int)a;
    ...
    for (b_r = 0; b_r < i; b_r++) {
      ...
      vlen = 0;
      for (sizes_idx_1 = 0; sizes_idx_1 < d_loop_ub; sizes_idx_1++) {
        d = C->data[b_r + C->size[0] * sizes_idx_1];
      ...
      loop_ub = r->size[1];
      for (i1 = 0; i1 < loop_ub; i1++) {
        Cr->data[r->data[i1] - 1] = C->data[b_r + C->size[0] * (r->data[i1] - 1)]
          + 1.0;
      }
      ...
      if (b_loop_ub < 1) {
        Cr->data[0] = randsample(lshift - 1.0);
      }
      ...
      loop_ub = Cr->size[1];
      for (nx = 0; nx < loop_ub; nx++) {
        b_C->data[b_r + b_C->size[0] * (i1 + nx)] = Cr->data[nx];
      }
    }
    ...
    subsets(varargin_22_data, varargin_22_size, y->size[0], pini, ncomb, C, &a);
    i = b_C->size[0] * b_C->size[1];
    b_C->size[0] = C->size[0];
    b_C->size[1] = C->size[1];
    emxEnsureCapacity_real_T(b_C, i);
  ...
  if (0 <= (int)a - 1) {
    ...
    f_loop_ub = b_sizes_idx_1;
    g_loop_ub = b_C->size[1];
    h_loop_ub = b_C->size[1];
    i_loop_ub = b_C->size[1];
  }
  ...
  for (sizes_idx_1 = 0; sizes_idx_1 < i; sizes_idx_1++) {
    ...
    for (i1 = 0; i1 < e_loop_ub; i1++) {
      for (nx = 0; nx < result; nx++) {
        Xfinal->data[nx + Xfinal->size[0] * i1] = Xsel.contents->data[nx +
          result * i1];
      }
    }
    ...
    for (i1 = 0; i1 < f_loop_ub; i1++) {
      for (nx = 0; nx < result; nx++) {
        Xfinal->data[nx + Xfinal->size[0] * b_input_sizes_idx_1] =
          Xlshift.contents->data[nx];
      }
    }
    ...
    b_Xfinal->size[0] = i_loop_ub;
    b_Xfinal->size[1] = Xfinal->size[1];
    emxEnsureCapacity_real_T(b_Xfinal, i1);
    for (i1 = 0; i1 < loop_ub; i1++) {
      for (nx = 0; nx < i_loop_ub; nx++) {
        b_Xfinal->data[nx + b_Xfinal->size[0] * i1] = Xfinal->data[((int)
          b_C->data[sizes_idx_1 + b_C->size[0] * nx] + Xfinal->size[0] * i1) - 1];
      }
    }
    ...
    if (!any(r1)) {
      ...
      if (ncomb > lts->bestr) {
        if (b_expl_temp.numscale2rw < sworst) {
          ...
          maximum(bestnumscale2, &sworst, &nx);
        ...
        bestnumscale2->data[(int)ncomb - 1] = b_expl_temp.numscale2rw;
        loop_ub = expl_temp.betarw->size[0];
        for (i1 = 0; i1 < loop_ub; i1++) {
          out->invXX->data[((int)ncomb + out->invXX->size[0] * i1) - 1] =
            expl_temp.betarw->data[i1];
        }

        loop_ub = yhat.contents->size[0];
        for (i1 = 0; i1 < loop_ub; i1++) {
          bestyhat->data[i1 + bestyhat->size[0] * ((int)ncomb - 1)] =
            yhat.contents->data[i1];
        }
  ...
  if (0 <= (int)lts->bestr - 1) {
    i3 = bestyhat->size[0];
    j_loop_ub = bestyhat->size[0];
    i4 = out->invXX->size[1];
    k_loop_ub = out->invXX->size[1];
  }
  ...
  x_tmp = sqrt(ncomb / b_h);
  i = yhatrob->size[0];
  yhatrob->size[0] = yin.contents->size[0];
  emxEnsureCapacity_real_T(yhatrob, i);
  loop_ub = yin.contents->size[0];
  for (i = 0; i < loop_ub; i++) {
    yhatrob->data[i] = (yin.contents->data[i] - yhatrob->data[i]) / x_tmp;
  }
  ...
  emxEnsureCapacity_real_T(b_y, i);
  for (loop_ub = 0; loop_ub < nx; loop_ub++) {
    b_y->data[loop_ub] = fabs(yhatrob->data[loop_ub]);
  }
  ...
  factor = x_tmp * factor * sqrt(corfactorRAW(yin.contents->size[0], b_h /
    (double)yin.contents->size[0]));
  if (lshift > 0.0) {
    /*  Compute the residuals locally just changing the position of the level */
    /*  shift */
    sworst = lshiftlocref->wlength;
    a = brobLSH->data[brobLSH->size[0] - 1] - lshiftlocref->wlength;
    ncomb = brobLSH->data[brobLSH->size[0] - 1] + lshiftlocref->wlength;
    if (rtIsNaN(a) || rtIsNaN(ncomb)) {
      i = Cr->size[0] * Cr->size[1];
      Cr->size[0] = 1;
      Cr->size[1] = 1;
      emxEnsureCapacity_real_T(Cr, i);
      ...
      Cr->size[0] = 1;
      Cr->size[1] = 0;
    } else if ((rtIsInf(a) || rtIsInf(ncomb)) && (a == ncomb)) {
      i = Cr->size[0] * Cr->size[1];
      Cr->size[0] = 1;
      Cr->size[1] = 1;
      emxEnsureCapacity_real_T(Cr, i);
      ...
      eml_float_colon(a, ncomb, Cr);
    }

    /*  Reduce width of tloc dinamically */
    /*  make sure that tloc is in the range LSHmin and LSHmax */
    while (((c_maximum(Cr) > lshift) || (minimum(Cr) < lshift)) && (!(sworst ==
             0.0))) {
      sworst--;
      a = brobLSH->data[brobLSH->size[0] - 1] - sworst;
      ncomb = brobLSH->data[brobLSH->size[0] - 1] + sworst;
      if (rtIsNaN(a) || rtIsNaN(ncomb)) {
        i = Cr->size[0] * Cr->size[1];
        Cr->size[0] = 1;
        Cr->size[1] = 1;
    ...
    for (sizes_idx_1 = 0; sizes_idx_1 < i; sizes_idx_1++) {
      sworst = Cr->data[0] + (double)sizes_idx_1;
      loop_ub = yhatrob->size[0];
      i1 = betaini->size[0];
      ...
      i1 = bestnumscale2->size[0];
      bestnumscale2->size[0] = (int)(sworst - 1.0) + (int)(((double)y->size[0] -
        sworst) + 1.0);
      emxEnsureCapacity_real_T(bestnumscale2, i1);
      ...
      loop_ub = (int)(((double)y->size[0] - sworst) + 1.0);
      for (i1 = 0; i1 < loop_ub; i1++) {
        bestnumscale2->data[i1 + (int)(sworst - 1.0)] = 1.0;
      }
      ...
      nx = betaini->size[0];
      for (loop_ub = 0; loop_ub < nx; loop_ub++) {
        b_y->data[loop_ub] = betaini->data[loop_ub] * betaini->data[loop_ub];
      }

      out->Likloc->data[sizes_idx_1 + out->Likloc->size[0]] = blockedSummation
        (b_y, b_y->size[0]);
      i1 = betaini->size[0];
      ...
      out->Likloc->data[sizes_idx_1 + out->Likloc->size[0] * 2] =
    ...
    b_minimum(bestnumscale2, &sworst, &nx);
  ...
  emxEnsureCapacity_real_T(b_y, i);
  for (loop_ub = 0; loop_ub < nx; loop_ub++) {
    b_y->data[loop_ub] = fabs(bestnumscale2->data[loop_ub]);
  }
  ...
  do {
    ...
    for (i1 = 0; i1 < hmin; i1++) {
      for (nx = 0; nx < loop_ub; nx++) {
        bestyhat->data[nx + bestyhat->size[0] * i1] = Xsel.contents->data[((int)
          betaini->data[nx] + Xsel.contents->size[0] * (i + i1)) - 1];
      }
    }
  ...
  if (fabs(factor) < 1.0E-7) {
    zscoreFS(bestnumscale2, betaini, &sworst, &factor);
    if (factor == 0.0) {
      b_zscoreFS(bestnumscale2, betaini, &sworst, &factor);
    }

    if (factor == 0.0) {
      c_zscoreFS(bestnumscale2, betaini, &sworst, &factor);
    }

    if (factor == 0.0) {
      nx = bestnumscale2->size[0];
      i = betaini->size[0];
      betaini->size[0] = bestnumscale2->size[0];
      emxEnsureCapacity_real_T(betaini, i);
      for (loop_ub = 0; loop_ub < nx; loop_ub++) {
        betaini->data[loop_ub] = fabs(bestnumscale2->data[loop_ub]);
      }

      sizes_idx_1 = 0;
      exitg2 = false;
      while ((!exitg2) && (sizes_idx_1 < 25)) {
        factor = prctile(betaini, (double)sizes_idx_1 + 75.0) - prctile(betaini,
          100.0 - ((double)sizes_idx_1 + 75.0));
        if (factor > 0.0) {
          exitg2 = true;
        } else {
          sizes_idx_1++;
  ...
  if (SmallSampleCor == 1.0) {
    ...
    if (yin.contents->size[0] < 50) {
      ...
      b_loop_ub = yin.contents->size[0];
    }

    ncomb = RobRegrSize(b_loop_ub, pini + lshiftYN.contents, bdp);
    nx = bestnumscale2->size[0];
    i = b_y->size[0];
    b_y->size[0] = bestnumscale2->size[0];
    emxEnsureCapacity_real_T(b_y, i);
  ...
  nx = bestnumscale2->size[0];
  for (loop_ub = 0; loop_ub < nx; loop_ub++) {
    bestnumscale2->data[loop_ub] = sqrt(bestnumscale2->data[loop_ub]);
  }
  ...
  emxEnsureCapacity_real_T(b_y, i);
  for (loop_ub = 0; loop_ub < nx; loop_ub++) {
    b_y->data[loop_ub] = fabs(beta0->data[loop_ub]);
  }
  ...
  if (weights.contents->size[0] == 0) {
    ...
    for (loop_ub = 2; loop_ub <= vlen; loop_ub++) {
      nx += weights.contents->data[loop_ub - 1];
    }
  }
  ...
  if (weights.contents->size[0] == 0) {
    ...
    b_loop_ub = weights.contents->data[0];
    for (loop_ub = 2; loop_ub <= vlen; loop_ub++) {
      b_loop_ub += weights.contents->data[loop_ub - 1];
    }
  }
  ...
  if (nx < yin.contents->size[0]) {
    ...
    a = norminv(0.5 * (sworst / (double)yin.contents->size[0] + 1.0));

    /* factor=1/sqrt(1-(2*a.*normpdf(a))./(2*normcdf(a)-1)); */
    /*  Apply small sample correction factor to reweighted estimate */
    /*  of sigma */
    factor = 1.0 / sqrt(1.0 - 2.0 * ((double)yin.contents->size[0] / sworst) * a
                        * (exp(-0.5 * a * a) / 2.5066282746310002)) * sqrt
      (corfactorREW(yin.contents->size[0], sworst / (double)yin.contents->size[0]));
  } else {
    factor = 1.0;
  }
  ...
  nx = bestnumscale2->size[0];
  for (loop_ub = 0; loop_ub < nx; loop_ub++) {
    b_y->data[loop_ub] = bestnumscale2->data[loop_ub] * bestnumscale2->
      data[loop_ub];
  }
  ...
  factor *= sqrt(blockedSummation(b_y, b_y->size[0]) / ((double)b_loop_ub - 1.0));
  if (!(factor == 0.0)) {
    loop_ub = bestnumscale2->size[0];
    for (i = 0; i < loop_ub; i++) {
      bestnumscale2->data[i] /= factor;
  ...
  for (loop_ub = 0; loop_ub < i; loop_ub++) {
    ncomb = eml_erfcore(-b_y->data[loop_ub] / 1.4142135623730951);
    p_all->data[loop_ub] = 0.5 * ncomb;
  }
  ...
  if (yxsave) {
    if (intercept) {
      ...
      nx = out->X->size[0] * out->X->size[1];
      out->X->size[0] = Xseldum->size[0];
      b_loop_ub = i1 - i;
      out->X->size[1] = b_loop_ub;
      emxEnsureCapacity_real_T(out->X, nx);
      for (i1 = 0; i1 < b_loop_ub; i1++) {
        for (nx = 0; nx < loop_ub; nx++) {
          out->X->data[nx + out->X->size[0] * i1] = Xseldum->data[nx +
            Xseldum->size[0] * (i + i1)];
        }
      }
    } else {
  ...
  if (varampl.contents > 0.0) {
    ...
    a = ((double)b_loop_ub - varampl.contents) + 1.0;
    if (nx < a) {
      posvarampl->size[0] = 1;
      posvarampl->size[1] = 0;
  ...
  string_string(lab_data, lab_size, tmp_data, varargin_22_size);
  ...
  array2table(b_out, tmp_data, varargin_22_size, &out->Btable.metaDim.length,
  ...
  if (seasonal.contents < 6.0) {
    ...
    ncomb = ((double)Xtrend.contents->size[1] + (double)nseaso) - 2.0;
    ...
    if (ncomb < 1.0) {
      ...
      i = selWithoutLastHarmonic_tmp->size[0] * selWithoutLastHarmonic_tmp->
        size[1];
      selWithoutLastHarmonic_tmp->size[0] = 1;
      selWithoutLastHarmonic_tmp->size[1] = (int)(ncomb - 1.0) + 1;
    ...
    if ((varampl.contents == 0.0) && (lshiftYN.contents == 0.0)) {
      ...
      emxEnsureCapacity_real_T(Xfinal, i);
      b_loop_ub = selWithoutLastHarmonic->size[1];
      for (i = 0; i < b_loop_ub; i++) {
        for (i1 = 0; i1 < loop_ub; i1++) {
      ...
      if (seasonal.contents == 0.0) {
        ...
        for (i = 0; i < loop_ub; i++) {
          for (i1 = 0; i1 < b_loop_ub; i1++) {
            Xseaso.contents->data[i1 + (vlen + 1) * i] = Xseaso.contents->
      ...
      if (seasonal.contents == 0.0) {
        ...
        b_do_vectors(Cr, posvarampl, selWithoutLastHarmonic, ia, &b_loop_ub);
        varampl.contents = 0.0;
      }
    ...
    out->LastHarmonicPval = 1.0 - fcdf((ncomb - x_tmp) / 2.0 / (x_tmp / sworst),
      sworst);
  } else {
