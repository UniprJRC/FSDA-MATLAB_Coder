...
void LXS_wrapper(const emxArray_real_T *y, const emxArray_real_T *X,
                 ...
                 struct0_T *out, emxArray_real_T *C)
{
  ...
  if (lmsopt == 1) {
     ...
     * estimate */
    s0 = ncomb * (1.0 / sqrt(1.0 - 2.0 * (n / h) * tsampling *
  ...
  if (fabs(s0) > 1.0E-7) {
    ...
    fp_875_n = (conflev + 1.0) / 2.0;
    ncomb = tinv(fp_875_n, 2.0 * n / asvar(h, n));
    ...
    if (rew) {
      ...
      if (loop_ub_tmp < n) {
        ...
        tsampling = norminv(0.5 * ((double)loop_ub_tmp / n + 1.0));
        /* factor=1/sqrt(1-(2*a.*normpdf(a))./(2*normcdf(a)-1)); */
