...
void LXS_wrapper1(const emxArray_real_T *y, const emxArray_real_T *X,
                  ...
                  bool intercept, const struct0_T *lms, bool msg, bool nocheck,
                  bool nomes, double nsamp, bool rew, bool yxsave,
                  struct1_T *out, emxArray_real_T *C)
{
  ...
  chkinputR(b_y, out->X, intercept, nocheck, &n, &fp_875_n);
  if (rtIsNaN(n)) {
  ...
  /*  coefficients */
  loop_ub_tmp = (int)fp_875_n;
  i = out->beta->size[0];
  out->beta->size[0] = (int)fp_875_n;
  ...
  if (h < fp_875_n + 1.0) {
    int2str(fp_875_n + 1.0, p.data, p.size);
  }
  ...
  bestbetas->size[1] = (int)fp_875_n;
  emxEnsureCapacity_real_T(bestbetas, i);
  nx = (int)lms->bestr * (int)fp_875_n;
  for (i = 0; i < nx; i++) {
  ...
  bestsubset->size[1] = (int)fp_875_n;
  ...
  subsets(tsampling, n, fp_875_n, ncomb, C, &nselected);
  ...
  for (b_i = 0; b_i < i; b_i++) {
    ...
    if ((!rtIsNaN(b->data[0])) && (!rtIsInf(b->data[0]))) {
      ...
      if (ij > lms->bestr) {
        if (fp_875_n < sworst) {
          ...
          out->residuals->data[nx - 1] = fp_875_n;
        ...
        out->residuals->data[(int)ij - 1] = fp_875_n;
        loop_ub = tmp_betarw->size[0];
        for (i1 = 0; i1 < loop_ub; i1++) {
    ...
    if (!nomes) {
      if (b_i + 1U <= (unsigned int)tsampling) {
        ...
        time_data[b_i] = toc(s0, ttic_tv_nsec);
  ...
  for (b_i = 0; b_i < b_loop_ub_tmp; b_i++) {
    ...
    IRWLSreg(b_y, out->X, outliers, lms->refstepsbestr, lms->reftolbestr, h,
             tmp_betarw, &fp_875_n, expl_temp);
    if (fp_875_n < ncomb) {
      /*  sh0 = superbestscale */
      tsampling = fp_875_n;
      /*  brob = superbestbeta */
  ...
  if (h < n) {
    ...
    tsampling = 1.0 - 1.2995580068132053 / rt_powd_snf(n, 0.604756680630497);
  ...
  if (fabs(s0) > 1.0E-7) {
    ...
    tsampling = (conflev + 1.0) / 2.0;
    ncomb = tinv(tsampling, 2.0 * n / asvar(h, n));
    ...
    if (rew) {
      ...
      if (loop_ub_tmp < n) {
        ...
        ncomb = norminv(0.5 * ((double)loop_ub_tmp / n + 1.0));
        /* factor=1/sqrt(1-(2*a.*normpdf(a))./(2*normcdf(a)-1)); */
        ...
        ncomb =
            ...
            sqrt(1.0 - 2.0 * (n / (double)loop_ub_tmp) * ncomb *
                           (exp(-0.5 * ncomb * ncomb) / 2.5066282746310002)) *
            sqrt(corfactorREW(n, (double)loop_ub_tmp / n));
      } else {
        ncomb = 1.0;
      }
      i = b->size[0];
