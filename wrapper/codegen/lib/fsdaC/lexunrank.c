 ...
 * pascal(n). In applications where lexunrank is called many times, it is
 * preferable to compute the Pascal matrix once outside lexunrank, and pass it
 *            to lexunrank as optional argument. Otherwise, the required
 * binomial coeffients are computed inside lexunrank using function bc and, when
