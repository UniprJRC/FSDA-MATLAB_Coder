             ...
             emxArray_real_T *d)
{
  ...
  emxInit_real_T(sp, &Ytilde, 2, &jt_emlrtRTEI, true);
  ...
  emxInit_real_T(&st, &r, 2, &kt_emlrtRTEI, true);
  emxInit_real_T(&st, &r1, 2, &it_emlrtRTEI, true);
  b_st.site = &lv_emlrtRSI;
  mrdiv(&b_st, Ytilde, SIGMA, r);
  iv[0] = (*(int32_T(*)[2])r->size)[0];
  iv[1] = (*(int32_T(*)[2])r->size)[1];
  iv1[0] = (*(int32_T(*)[2])Ytilde->size)[0];
  iv1[1] = (*(int32_T(*)[2])Ytilde->size)[1];
  emlrtSizeEqCheckNDR2012b(&iv[0], &iv1[0], &mc_emlrtECI, (emlrtCTX)sp);
  i = r1->size[0] * r1->size[1];
  r1->size[0] = r->size[0];
  r1->size[1] = r->size[1];
  emxEnsureCapacity_real_T(sp, r1, i, &it_emlrtRTEI);
