...
void quantile(const emlrtStack *sp, const emxArray_real_T *x, real_T y[3])
{
  ...
  if (x->size[0] == 0) {
    ...
    emxInit_int32_T(&b_st, &idx, 1, &rf_emlrtRTEI, true);
    ...
    while ((!exitg1) && (nj > 0)) {
      if (nj > idx->size[0]) {
        emlrtDynamicBoundsCheckR2012b(nj, 1, idx->size[0], &bk_emlrtBCI, &c_st);
      }
      i = idx->data[nj - 1];
      if ((i < 1) || (i > x->size[0])) {
        emlrtDynamicBoundsCheckR2012b(i, 1, x->size[0], &bk_emlrtBCI, &c_st);
      }
    ...
    if (nj < 1) {
      ...
      if ((idx->data[0] < 1) || (idx->data[0] > x->size[0])) {
        emlrtDynamicBoundsCheckR2012b(idx->data[0], 1, x->size[0], &gk_emlrtBCI,
                                      &b_st);
      }
      ...
      if (b_i >= nj) {
        if (nj > idx->size[0]) {
          emlrtDynamicBoundsCheckR2012b(nj, 1, idx->size[0], &dk_emlrtBCI,
                                        &b_st);
        }
        i = idx->data[nj - 1];
        if ((i < 1) || (i > x->size[0])) {
          emlrtDynamicBoundsCheckR2012b(i, 1, x->size[0], &dk_emlrtBCI, &b_st);
        }
        y[0] = x->data[i - 1];
      } else {
        r -= (real_T)b_i;
        ...
        i = idx->data[b_i - 1];
        if ((i < 1) || (i > x->size[0])) {
          emlrtDynamicBoundsCheckR2012b(i, 1, x->size[0], &ek_emlrtBCI, &b_st);
        }
        if (b_i + 1 > idx->size[0]) {
        ...
        if ((b_i < 1) || (b_i > x->size[0])) {
          emlrtDynamicBoundsCheckR2012b(b_i, 1, x->size[0], &fk_emlrtBCI,
                                        &b_st);
        }
