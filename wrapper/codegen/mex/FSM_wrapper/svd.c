...
void svd(const emlrtStack *sp, const emxArray_real_T *A, emxArray_real_T *U)
{
  ...
  b_A->size[0] = A->size[0];
  b_A->size[1] = A->size[1];
  emxEnsureCapacity_real_T(&c_st, b_A, n, &xd_emlrtRTEI);
  m = A->size[0] * A->size[1];
  for (n = 0; n < m; n++) {
    b_A->data[n] = A->data[n];
  }
  ...
  n = U->size[0];
  U->size[0] = m;
  emxEnsureCapacity_real_T(&c_st, U, n, &yd_emlrtRTEI);
  if ((A->size[0] != 0) && (A->size[1] != 0)) {
    emxInit_real_T(&c_st, &superb, 1, &ce_emlrtRTEI, true);
    if (m > 1) {
      n = superb->size[0];
      superb->size[0] = m - 1;
      emxEnsureCapacity_real_T(&c_st, superb, n, &be_emlrtRTEI);
    } else {
      n = superb->size[0];
      superb->size[0] = 1;
      emxEnsureCapacity_real_T(&c_st, superb, n, &ae_emlrtRTEI);
    }
