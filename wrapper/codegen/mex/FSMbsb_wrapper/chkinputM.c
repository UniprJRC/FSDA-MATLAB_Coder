...
void chkinputM(const emlrtStack *sp, emxArray_real_T *X, boolean_T vvarargin_f8)
{
  ...
  if (!vvarargin_f8) {
    ...
    emxInit_real_T(sp, &y, 1, &si_emlrtRTEI, true);
    emxInit_real_T(sp, &b, 1, &gi_emlrtRTEI, true);
    /*  Check dimension consistency of X and y */
    st.site = &cc_emlrtRSI;
    i = b->size[0];
    b->size[0] = X->size[1];
    emxEnsureCapacity_real_T(&st, b, i, &gi_emlrtRTEI);
    loop_ub = X->size[1];
    for (i = 0; i < loop_ub; i++) {
      b->data[i] = 1.0;
    }
    ...
    if ((X->size[0] == 0) || (X->size[1] == 0) || (X->size[1] == 0)) {
      i = y->size[0];
      y->size[0] = X->size[0];
      emxEnsureCapacity_real_T(&b_st, y, i, &hi_emlrtRTEI);
      loop_ub = X->size[0];
      for (i = 0; i < loop_ub; i++) {
        y->data[i] = 0.0;
      }
      ...
      i = y->size[0];
      y->size[0] = X->size[0];
      emxEnsureCapacity_real_T(&d_st, y, i, &qg_emlrtRTEI);
      dgemm(&TRANSA1, &TRANSB1, &m_t, &n_t, &k_t, &alpha1, &X->data[0], &lda_t,
            &b->data[0], &ldb_t, &beta1, &y->data[0], &ldc_t);
    }
    emxFree_real_T(&b);
    emxInit_boolean_T(&b_st, &ok, 1, &qi_emlrtRTEI, true);
    /*  Observations with missing or infinite values are removed from X and y */
    i = ok->size[0];
    ok->size[0] = y->size[0];
    emxEnsureCapacity_boolean_T(sp, ok, i, &ii_emlrtRTEI);
    loop_ub = y->size[0];
    for (i = 0; i < loop_ub; i++) {
      ok->data[i] = muDoubleScalarIsInf(y->data[i]);
    }
    emxInit_boolean_T(sp, &r, 1, &ji_emlrtRTEI, true);
    i = r->size[0];
    r->size[0] = y->size[0];
    emxEnsureCapacity_boolean_T(sp, r, i, &ji_emlrtRTEI);
    loop_ub = y->size[0];
    for (i = 0; i < loop_ub; i++) {
    ...
    emxInit_int32_T(sp, &r1, 1, &ri_emlrtRTEI, true);
    i = r1->size[0];
    r1->size[0] = b_m;
    emxEnsureCapacity_int32_T(sp, r1, i, &ki_emlrtRTEI);
    b_m = 0;
    for (i = 0; i <= n; i++) {
      if (ok->data[i]) {
        r1->data[b_m] = i + 1;
    ...
    emxInit_real_T(sp, &b_X, 2, &li_emlrtRTEI, true);
    b_m = X->size[0];
    n = X->size[1] - 1;
    i = b_X->size[0] * b_X->size[1];
    b_X->size[0] = r1->size[0];
    b_X->size[1] = n + 1;
    emxEnsureCapacity_real_T(sp, b_X, i, &li_emlrtRTEI);
    for (i = 0; i <= n; i++) {
      loop_ub = r1->size[0];
      for (b_n = 0; b_n < loop_ub; b_n++) {
        if (r1->data[b_n] > b_m) {
          emlrtDynamicBoundsCheckR2012b(r1->data[b_n], 1, b_m, &cd_emlrtBCI,
                                        (emlrtCTX)sp);
        }
    ...
    X->size[0] = b_X->size[0];
    X->size[1] = b_X->size[1];
    emxEnsureCapacity_real_T(sp, X, i, &mi_emlrtRTEI);
    loop_ub = b_X->size[1];
    for (i = 0; i < loop_ub; i++) {
      b_m = b_X->size[0];
    ...
    emxInit_real_T(&d_st, &constcols, 2, &oi_emlrtRTEI, true);
    ...
    emxInit_real_T(&d_st, &r2, 2, &ki_emlrtRTEI, true);
    e_st.site = &rc_emlrtRSI;
    f_st.site = &sc_emlrtRSI;
    g_st.site = &tc_emlrtRSI;
    b_m = X->size[0];
    ...
    emxInit_boolean_T(&g_st, &b_constcols, 2, &ni_emlrtRTEI, true);
    ...
    emxEnsureCapacity_boolean_T(&st, b_constcols, i, &ni_emlrtRTEI);
    loop_ub = constcols->size[1];
    for (i = 0; i < loop_ub; i++) {
      b_constcols->data[i] = (constcols->data[i] - r2->data[i] == 0.0);
    }
    emxFree_real_T(&r2);
    emxInit_int32_T(&st, &ii, 2, &nf_emlrtRTEI, true);
    b_st.site = &ad_emlrtRSI;
    eml_find(&b_st, b_constcols, ii);
    i = constcols->size[0] * constcols->size[1];
    constcols->size[0] = 1;
    constcols->size[1] = ii->size[1];
    emxEnsureCapacity_real_T(&st, constcols, i, &oi_emlrtRTEI);
    loop_ub = ii->size[1];
    ...
    if (constcols->size[1] >= 1) {
      ...
      emxEnsureCapacity_int32_T(sp, ii, i, &pi_emlrtRTEI);
      for (i = 0; i < loop_ub; i++) {
