...
int32_T local_rank(const emlrtStack *sp, const emxArray_real_T *A)
{
  ...
  if ((A->size[0] != 0) && (A->size[1] != 0)) {
    ...
    emxInit_real_T(&st, &s, 1, &sf_emlrtRTEI, true);
    if (p) {
      b_st.site = &ud_emlrtRSI;
      svd(&b_st, A, s);
    } else {
      emxInit_real_T(&st, &r, 2, &rf_emlrtRTEI, true);
      k = r->size[0] * r->size[1];
      r->size[0] = A->size[0];
      r->size[1] = A->size[1];
      emxEnsureCapacity_real_T(&st, r, k, &rf_emlrtRTEI);
      nx = A->size[0] * A->size[1];
      for (k = 0; k < nx; k++) {
        r->data[k] = 0.0;
      }
      ...
      k = s->size[0];
      s->size[0] = nx;
      emxEnsureCapacity_real_T(&st, s, k, &sf_emlrtRTEI);
      emxFree_real_T(&r);
      for (k = 0; k < nx; k++) {
        s->data[k] = rtNaN;
      }
