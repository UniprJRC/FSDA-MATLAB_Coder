...
void inv(const emlrtStack *sp, const emxArray_real_T *x, emxArray_real_T *y)
{
  ...
  if (x->size[0] != x->size[1]) {
    emlrtErrorWithMessageIdR2018a(sp, &ic_emlrtRTEI, "Coder:MATLAB:square",
  ...
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    ...
    y->size[0] = x->size[0];
    y->size[1] = x->size[1];
    emxEnsureCapacity_real_T(sp, y, i, &gr_emlrtRTEI);
    ...
    y->size[0] = x->size[0];
    y->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&st, y, i, &hr_emlrtRTEI);
    yk = x->size[0] * x->size[1];
    for (i = 0; i < yk; i++) {
      y->data[i] = 0.0;
    }
    emxInit_real_T(&st, &b_x, 2, &nr_emlrtRTEI, true);
    b_st.site = &ow_emlrtRSI;
    i = b_x->size[0] * b_x->size[1];
    b_x->size[0] = x->size[0];
    b_x->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&b_st, b_x, i, &ir_emlrtRTEI);
    yk = x->size[0] * x->size[1];
    for (i = 0; i < yk; i++) {
      b_x->data[i] = x->data[i];
    }
    emxInit_int32_T(&b_st, &ipiv, 2, &nr_emlrtRTEI, true);
    emxInit_ptrdiff_t(&b_st, &ipiv_t, 1, &or_emlrtRTEI, true);
    emxInit_ptrdiff_t(&b_st, &r, 1, &jr_emlrtRTEI, true);
    c_st.site = &uw_emlrtRSI;
    d_st.site = &ww_emlrtRSI;
    repmat(&d_st, (ptrdiff_t)0.0, muIntScalarMin_sint32(n, n), r);
    i = ipiv_t->size[0];
    ipiv_t->size[0] = r->size[0];
    emxEnsureCapacity_ptrdiff_t(&c_st, ipiv_t, i, &jr_emlrtRTEI);
    ...
    emxEnsureCapacity_int32_T(&c_st, ipiv, i, &kr_emlrtRTEI);
    ...
    if (yk < 0) {
      if (yk == -1010) {
        ...
        emlrtErrorWithMessageIdR2018a(
            &d_st, &r_emlrtRTEI, "Coder:toolbox:LAPACKCallErrorInfo",
            "Coder:toolbox:LAPACKCallErrorInfo", 5, 4, 19, &cv17[0], 12, yk);
      }
    }
    i = ipiv_t->size[0] - 1;
    for (k = 0; k <= i; k++) {
    ...
    emxInit_int32_T(&c_st, &p, 2, &mr_emlrtRTEI, true);
    b_st.site = &pw_emlrtRSI;
    c_st.site = &fx_emlrtRSI;
    d_st.site = &fd_emlrtRSI;
    e_st.site = &bx_emlrtRSI;
    f_st.site = &cx_emlrtRSI;
    g_st.site = &dx_emlrtRSI;
    b_n = x->size[0];
    i = p->size[0] * p->size[1];
    p->size[0] = 1;
    p->size[1] = x->size[0];
    emxEnsureCapacity_int32_T(&f_st, p, i, &lr_emlrtRTEI);
    p->data[0] = 1;
    yk = 1;
    g_st.site = &ex_emlrtRSI;
    ...
    if ((n1x == 0.0) || (n1xinv == 0.0) || (rc == 0.0)) {
      ...
      i_emlrt_marshallIn(&c_st, d_sprintf(&c_st, b_y, c_y, &u_emlrtMCI),
