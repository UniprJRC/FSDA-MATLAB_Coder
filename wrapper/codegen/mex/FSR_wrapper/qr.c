        ...
        emxArray_real_T *R)
{
  ...
  emxInit_real_T(sp, &b_A, 2, &wr_emlrtRTEI, true);
  st.site = &kx_emlrtRSI;
  b_st.site = &lx_emlrtRSI;
  i = b_A->size[0] * b_A->size[1];
  b_A->size[0] = A->size[0];
  b_A->size[1] = A->size[1];
  emxEnsureCapacity_real_T(&b_st, b_A, i, &qr_emlrtRTEI);
  na = A->size[0] * A->size[1];
  for (i = 0; i < na; i++) {
    b_A->data[i] = A->data[i];
  }
  emxInit_real_T(&b_st, &tau, 1, &wr_emlrtRTEI, true);
  ...
  i = tau->size[0];
  tau->size[0] = minmana;
  emxEnsureCapacity_real_T(&d_st, tau, i, &rr_emlrtRTEI);
  if ((b_A->size[0] == 0) || (b_A->size[1] == 0)) {
    i = tau->size[0];
    tau->size[0] = minmana;
    emxEnsureCapacity_real_T(&d_st, tau, i, &sr_emlrtRTEI);
    for (i = 0; i < minmana; i++) {
      tau->data[i] = 0.0;
    }
  } else {
      ...
      if (na != -4) {
        if (na == -1010) {
          ...
          emlrtErrorWithMessageIdR2018a(
              &e_st, &r_emlrtRTEI, "Coder:toolbox:LAPACKCallErrorInfo",
  ...
  R->size[0] = minmana;
  R->size[1] = b_A->size[1];
  emxEnsureCapacity_real_T(&b_st, R, i, &tr_emlrtRTEI);
  ...
  if ((b_A->size[0] != 0) && (b_A->size[1] != 0)) {
    ...
    if (p) {
      ...
      b_A->size[0] = na;
      b_A->size[1] = ma;
      emxEnsureCapacity_real_T(&e_st, b_A, i, &vr_emlrtRTEI);
      na *= ma;
      for (i = 0; i < na; i++) {
        b_A->data[i] = rtNaN;
      }
  ...
  Q->size[0] = m;
  Q->size[1] = minmana;
  emxEnsureCapacity_real_T(&b_st, Q, i, &ur_emlrtRTEI);
