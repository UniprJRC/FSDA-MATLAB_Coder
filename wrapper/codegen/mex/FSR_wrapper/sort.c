...
void c_sort(const emlrtStack *sp, emxArray_real_T *x)
{
  ...
  emxInit_real_T(sp, &vwork, 1, &shb_emlrtRTEI, true);
  if (dim + 2 <= 1) {
    i = x->size[0];
  } else {
    i = 1;
  ...
  vwork->size[0] = i;
  emxEnsureCapacity_real_T(sp, vwork, vstride, &qhb_emlrtRTEI);
  st.site = &kl_emlrtRSI;
  vstride = 1;
  for (k = 0; k <= dim; k++) {
    vstride *= x->size[0];
  ...
  emxInit_int32_T(sp, &dqb_emlrtRSI, 1, &thb_emlrtRTEI, true);
...
void sort(const emlrtStack *sp, emxArray_real_T *x, emxArray_int32_T *idx)
{
  ...
  i1 = vwork->size[0];
  vwork->size[0] = i;
  emxEnsureCapacity_real_T(sp, vwork, i1, &qhb_emlrtRTEI);
  i1 = idx->size[0];
  idx->size[0] = x->size[0];
  emxEnsureCapacity_int32_T(sp, idx, i1, &rhb_emlrtRTEI);
  st.site = &kl_emlrtRSI;
  vstride = 1;
  for (k = 0; k <= dim; k++) {
    vstride *= x->size[0];
  ...
  emxInit_int32_T(sp, &iidx, 1, &thb_emlrtRTEI, true);
  for (dim = 0; dim < vstride; dim++) {
    st.site = &nl_emlrtRSI;
    if ((1 <= i) && (i > 2147483646)) {
      b_st.site = &ab_emlrtRSI;
