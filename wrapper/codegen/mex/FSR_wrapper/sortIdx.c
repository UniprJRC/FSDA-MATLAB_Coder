...
void b_sortIdx(const emlrtStack *sp, emxArray_real_T *x, emxArray_int32_T *idx)
{
  ...
  idx->size[0] = ib;
  emxEnsureCapacity_int32_T(sp, idx, quartetOffset, &ok_emlrtRTEI);
  for (quartetOffset = 0; quartetOffset < ib; quartetOffset++) {
    idx->data[quartetOffset] = 0;
  }
  if (x->size[0] != 0) {
    emxInit_int32_T(sp, &iwork, 1, &whb_emlrtRTEI, true);
    st.site = &rl_emlrtRSI;
    n = x->size[0];
    ...
    iwork->size[0] = ib;
    emxEnsureCapacity_int32_T(&b_st, iwork, quartetOffset, &uhb_emlrtRTEI);
    for (quartetOffset = 0; quartetOffset < ib; quartetOffset++) {
      iwork->data[quartetOffset] = 0;
    }
    emxInit_real_T(&b_st, &xwork, 1, &xhb_emlrtRTEI, true);
    ib = x->size[0];
    quartetOffset = xwork->size[0];
    xwork->size[0] = ib;
    emxEnsureCapacity_real_T(&b_st, xwork, quartetOffset, &vhb_emlrtRTEI);
    for (quartetOffset = 0; quartetOffset < ib; quartetOffset++) {
             ...
             emxArray_int32_T *idx)
{
  ...
  i = idx->size[0];
  idx->size[0] = x->size[0];
  emxEnsureCapacity_int32_T(sp, idx, i, &ok_emlrtRTEI);
  b_i = x->size[0];
  for (i = 0; i < b_i; i++) {
    idx->data[i] = 0;
  }
  emxInit_int32_T(sp, &iwork, 1, &qk_emlrtRTEI, true);
  st.site = &lk_emlrtRSI;
  i = iwork->size[0];
  iwork->size[0] = x->size[0];
  emxEnsureCapacity_int32_T(&st, iwork, i, &pk_emlrtRTEI);
  b_i = x->size[0] - 1;
