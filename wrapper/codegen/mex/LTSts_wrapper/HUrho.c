           ...
           emxArray_real_T *rhoHU)
{
  ...
  emxInit_boolean_T(sp, &w, 1, &us_emlrtRTEI, true);
  emxInit_real_T(sp, &b, 1, &ws_emlrtRTEI, true);
  ...
  i = w->size[0];
  w->size[0] = b->size[0];
  emxEnsureCapacity_boolean_T(sp, w, i, &us_emlrtRTEI);
  loop_ub = b->size[0];
  for (i = 0; i < loop_ub; i++) {
    w->data[i] = (b->data[i] <= c);
  }
  ...
  i = b->size[0];
  b->size[0] = w->size[0];
  emxEnsureCapacity_real_T(sp, b, i, &vs_emlrtRTEI);
  loop_ub = w->size[0];
  for (i = 0; i < loop_ub; i++) {
