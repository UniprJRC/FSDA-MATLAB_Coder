...
void mrdiv(const emlrtStack *sp, emxArray_real_T *A, const emxArray_real_T *B)
{
  ...
  emxInit_real_T(sp, &Y, 2, &qy_emlrtRTEI, true);
  emxInit_real_T(sp, &b_B, 2, &ny_emlrtRTEI, true);
  emxInit_real_T(sp, &b_A, 2, &ry_emlrtRTEI, true);
  emxInit_real_T(sp, &tau, 1, &ry_emlrtRTEI, true);
  emxInit_int32_T(sp, &jpvt, 2, &ry_emlrtRTEI, true);
  emxInit_ptrdiff_t(sp, &ipiv_t, 1, &qp_emlrtRTEI, true);
  emxInit_ptrdiff_t(sp, &r, 1, &lp_emlrtRTEI, true);
  if ((A->size[0] == 0) || (A->size[1] == 0) ||
      ((B->size[0] == 0) || (B->size[1] == 0))) {
    m = A->size[0];
    i = A->size[0] * A->size[1];
    A->size[1] = B->size[0];
    emxEnsureCapacity_real_T(sp, A, i, &my_emlrtRTEI);
    nb = B->size[0];
    for (i = 0; i < nb; i++) {
    ...
    b_A->size[0] = B->size[0];
    b_A->size[1] = B->size[1];
    emxEnsureCapacity_real_T(&d_st, b_A, i, &kp_emlrtRTEI);
    ...
    emxEnsureCapacity_ptrdiff_t(&e_st, ipiv_t, i, &lp_emlrtRTEI);
    ...
    emxEnsureCapacity_int32_T(&e_st, jpvt, i, &mp_emlrtRTEI);
    ...
    b_B->size[0] = A->size[1];
    b_B->size[1] = A->size[0];
    emxEnsureCapacity_real_T(&st, b_B, i, &ny_emlrtRTEI);
    nb = A->size[0];
    for (i = 0; i < nb; i++) {
      info = A->size[1];
    ...
    b_A->size[0] = B->size[1];
    b_A->size[1] = B->size[0];
    emxEnsureCapacity_real_T(&st, b_A, i, &pw_emlrtRTEI);
    nb = B->size[0];
    for (i = 0; i < nb; i++) {
      info = B->size[1];
      for (a_tmp = 0; a_tmp < info; a_tmp++) {
    ...
    Y->size[0] = b_A->size[1];
    Y->size[1] = b_B->size[1];
    emxEnsureCapacity_real_T(&b_st, Y, i, &oy_emlrtRTEI);
    ...
    if ((b_A->size[0] != 0) && (b_A->size[1] != 0) &&
      ...
      if (infocheck(&e_st, info)) {
        if ((info == -10) && (b_B->size[1] > 1)) {
          ...
          b_B->size[0] = m;
          b_B->size[1] = info;
          emxEnsureCapacity_real_T(&d_st, b_B, i, &tw_emlrtRTEI);
          nb = m * info;
          for (i = 0; i < nb; i++) {
            b_B->data[i] = rtNaN;
          }
    ...
    A->size[0] = Y->size[1];
    A->size[1] = Y->size[0];
    emxEnsureCapacity_real_T(sp, A, i, &py_emlrtRTEI);
    nb = Y->size[0];
    for (i = 0; i < nb; i++) {
      info = Y->size[1];
      for (a_tmp = 0; a_tmp < info; a_tmp++) {
