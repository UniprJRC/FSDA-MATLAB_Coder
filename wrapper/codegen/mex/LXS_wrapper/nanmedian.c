...
real_T nanmedian(const emlrtStack *sp, const emxArray_real_T *x)
{
  ...
  emxInit_int32_T(sp, &idx, 1, &nj_emlrtRTEI, true);
  st.site = &ik_emlrtRSI;
  ...
  while ((!exitg1) && (nj > 0)) {
    if (nj > idx->size[0]) {
      emlrtDynamicBoundsCheckR2012b(nj, 1, idx->size[0], &ll_emlrtBCI, &d_st);
    }
    i = idx->data[nj - 1];
    if ((i < 1) || (i > x->size[0])) {
      emlrtDynamicBoundsCheckR2012b(i, 1, x->size[0], &ll_emlrtBCI, &d_st);
    }
  ...
  if (nj < 1) {
    ...
    if ((idx->data[0] < 1) || (idx->data[0] > x->size[0])) {
      emlrtDynamicBoundsCheckR2012b(idx->data[0], 1, x->size[0], &ml_emlrtBCI,
                                    &c_st);
    }
    ...
    if (b_i >= nj) {
      if (nj > idx->size[0]) {
        emlrtDynamicBoundsCheckR2012b(nj, 1, idx->size[0], &nl_emlrtBCI, &c_st);
      }
      i = idx->data[nj - 1];
      if ((i < 1) || (i > x->size[0])) {
        emlrtDynamicBoundsCheckR2012b(i, 1, x->size[0], &nl_emlrtBCI, &c_st);
      }
      y = x->data[i - 1];
    } else {
      r -= (real_T)b_i;
      if (b_i > idx->size[0]) {
        emlrtDynamicBoundsCheckR2012b(b_i, 1, idx->size[0], &ol_emlrtBCI,
                                      &c_st);
      }
      i = idx->data[b_i - 1];
      if ((i < 1) || (i > x->size[0])) {
        emlrtDynamicBoundsCheckR2012b(i, 1, x->size[0], &ol_emlrtBCI, &c_st);
      }
      if (b_i + 1 > idx->size[0]) {
        emlrtDynamicBoundsCheckR2012b(b_i + 1, 1, idx->size[0], &pl_emlrtBCI,
                                      &c_st);
      }
      nj = idx->data[b_i];
      if ((nj < 1) || (nj > x->size[0])) {
        emlrtDynamicBoundsCheckR2012b(nj, 1, x->size[0], &pl_emlrtBCI, &c_st);
      }
