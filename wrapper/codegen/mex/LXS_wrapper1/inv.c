...
void inv(const emlrtStack *sp, const emxArray_real_T *x, emxArray_real_T *y)
{
  ...
  if ((x->size[0] == 0) || (x->size[1] == 0)) {
    ...
    y->size[0] = x->size[0];
    y->size[1] = x->size[1];
    emxEnsureCapacity_real_T(sp, y, i, &op_emlrtRTEI);
    ...
    y->size[0] = x->size[0];
    y->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&st, y, i, &pp_emlrtRTEI);
    yk = x->size[0] * x->size[1];
    for (i = 0; i < yk; i++) {
      y->data[i] = 0.0;
    }
    emxInit_real_T(&st, &b_x, 2, &vp_emlrtRTEI, true);
    b_st.site = &jw_emlrtRSI;
    i = b_x->size[0] * b_x->size[1];
    b_x->size[0] = x->size[0];
    b_x->size[1] = x->size[1];
    emxEnsureCapacity_real_T(&b_st, b_x, i, &qp_emlrtRTEI);
    yk = x->size[0] * x->size[1];
    for (i = 0; i < yk; i++) {
      b_x->data[i] = x->data[i];
    }
    emxInit_int32_T(&b_st, &ipiv, 2, &vp_emlrtRTEI, true);
    emxInit_ptrdiff_t(&b_st, &ipiv_t, 1, &wp_emlrtRTEI, true);
    emxInit_ptrdiff_t(&b_st, &r, 1, &rp_emlrtRTEI, true);
    c_st.site = &pw_emlrtRSI;
    d_st.site = &rw_emlrtRSI;
    repmat(&d_st, (ptrdiff_t)0.0, muIntScalarMin_sint32(n, n), r);
    i = ipiv_t->size[0];
    ipiv_t->size[0] = r->size[0];
    emxEnsureCapacity_ptrdiff_t(&c_st, ipiv_t, i, &rp_emlrtRTEI);
    ...
    emxEnsureCapacity_int32_T(&c_st, ipiv, i, &sp_emlrtRTEI);
    ...
    emxInit_int32_T(&c_st, &p, 2, &up_emlrtRTEI, true);
    b_st.site = &kw_emlrtRSI;
    c_st.site = &ax_emlrtRSI;
    d_st.site = &ee_emlrtRSI;
    e_st.site = &vw_emlrtRSI;
    f_st.site = &ww_emlrtRSI;
    g_st.site = &xw_emlrtRSI;
    b_n = x->size[0];
    i = p->size[0] * p->size[1];
    p->size[0] = 1;
    p->size[1] = x->size[0];
    emxEnsureCapacity_int32_T(&f_st, p, i, &tp_emlrtRTEI);
    p->data[0] = 1;
    yk = 1;
