...
real_T nanmedian(const emlrtStack *sp, const emxArray_real_T *x)
{
  ...
  emxInit_int32_T(sp, &idx, 1, &tj_emlrtRTEI, true);
  st.site = &gk_emlrtRSI;
  ...
  while ((!exitg1) && (nj > 0)) {
    if (nj > idx->size[0]) {
      emlrtDynamicBoundsCheckR2012b(nj, 1, idx->size[0], &rl_emlrtBCI, &d_st);
    }
    i = idx->data[nj - 1];
    if ((i < 1) || (i > x->size[0])) {
      emlrtDynamicBoundsCheckR2012b(i, 1, x->size[0], &rl_emlrtBCI, &d_st);
    }
  ...
  if (nj < 1) {
    ...
    if ((idx->data[0] < 1) || (idx->data[0] > x->size[0])) {
      emlrtDynamicBoundsCheckR2012b(idx->data[0], 1, x->size[0], &sl_emlrtBCI,
                                    &c_st);
    }
    ...
    if (b_i >= nj) {
      ...
      i = idx->data[b_i - 1];
      if ((i < 1) || (i > x->size[0])) {
        emlrtDynamicBoundsCheckR2012b(i, 1, x->size[0], &ul_emlrtBCI, &c_st);
      }
      if (b_i + 1 > idx->size[0]) {
      ...
      nj = idx->data[b_i];
      if ((nj < 1) || (nj > x->size[0])) {
        emlrtDynamicBoundsCheckR2012b(nj, 1, x->size[0], &vl_emlrtBCI, &c_st);
      }
