        ...
        emxArray_real_T *R)
{
  ...
  emxInit_real_T(sp, &b_A, 2, &gq_emlrtRTEI, true);
  st.site = &gx_emlrtRSI;
  b_st.site = &hx_emlrtRSI;
  i = b_A->size[0] * b_A->size[1];
  b_A->size[0] = A->size[0];
  b_A->size[1] = A->size[1];
  emxEnsureCapacity_real_T(&b_st, b_A, i, &aq_emlrtRTEI);
  na = A->size[0] * A->size[1];
  for (i = 0; i < na; i++) {
    b_A->data[i] = A->data[i];
  }
  emxInit_real_T(&b_st, &tau, 1, &gq_emlrtRTEI, true);
  ...
  i = tau->size[0];
  tau->size[0] = minmana;
  emxEnsureCapacity_real_T(&d_st, tau, i, &bq_emlrtRTEI);
  if ((b_A->size[0] == 0) || (b_A->size[1] == 0)) {
    i = tau->size[0];
    tau->size[0] = minmana;
    emxEnsureCapacity_real_T(&d_st, tau, i, &cq_emlrtRTEI);
    for (i = 0; i < minmana; i++) {
      tau->data[i] = 0.0;
    }
  } else {
      ...
      if (na != -4) {
        if (na == -1010) {
          ...
          emlrtErrorWithMessageIdR2018a(
              &e_st, &o_emlrtRTEI, "Coder:toolbox:LAPACKCallErrorInfo",
  ...
  R->size[0] = minmana;
  R->size[1] = b_A->size[1];
  emxEnsureCapacity_real_T(&b_st, R, i, &dq_emlrtRTEI);
  ...
  if ((b_A->size[0] != 0) && (b_A->size[1] != 0)) {
    ...
    if (na != 0) {
      ...
      if (!b_p) {
        if (na == -1010) {
          ...
          emlrtErrorWithMessageIdR2018a(&f_st, &o_emlrtRTEI,
    ...
    if (p) {
      ...
      b_A->size[0] = na;
      b_A->size[1] = ma;
      emxEnsureCapacity_real_T(&e_st, b_A, i, &fq_emlrtRTEI);
      na *= ma;
      for (i = 0; i < na; i++) {
        b_A->data[i] = rtNaN;
      }
  ...
  Q->size[0] = m;
  Q->size[1] = minmana;
  emxEnsureCapacity_real_T(&b_st, Q, i, &eq_emlrtRTEI);
