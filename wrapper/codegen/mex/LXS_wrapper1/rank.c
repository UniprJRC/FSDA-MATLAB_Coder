...
int32_T local_rank(const emlrtStack *sp, const emxArray_real_T *A)
{
  ...
  if ((A->size[0] != 0) && (A->size[1] != 0)) {
    ...
    emxInit_real_T(&st, &s, 1, &qe_emlrtRTEI, true);
    if (p) {
      b_st.site = &qd_emlrtRSI;
      svd(&b_st, A, s);
    } else {
      emxInit_real_T(&st, &r, 2, &pe_emlrtRTEI, true);
      i = r->size[0] * r->size[1];
      r->size[0] = A->size[0];
      r->size[1] = A->size[1];
      emxEnsureCapacity_real_T(&st, r, i, &pe_emlrtRTEI);
      ...
      i = s->size[0];
      s->size[0] = loop_ub;
      emxEnsureCapacity_real_T(&st, s, i, &qe_emlrtRTEI);
      emxFree_real_T(&r);
      for (i = 0; i < loop_ub; i++) {
        s->data[i] = rtNaN;
      }
