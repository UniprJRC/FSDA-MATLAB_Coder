...
static void k_emlrt_marshallIn(const emlrtStack *sp, const mxArray *la, const
  ...
  u_size[1])
{
  ...
  int32_T i;
  y = NULL;
  m = emlrtCreateNumericArray(1, &u_size[0], mxDOUBLE_CLASS, mxREAL);
...
void addt_wrapper_api(const mxArray * const prhs[6], const mxArray *plhs[1])
{
  ...
  emxInit_real_T(&st, &y, 1, &mc_emlrtRTEI, true);
  emxInit_real_T(&st, &X, 2, &mc_emlrtRTEI, true);
  emxInit_real_T(&st, &w, 1, &mc_emlrtRTEI, true);
  ...
  addt_wrapper(&st, y, X, w, intercept, *la_data, la_size, nocheck, &out);
